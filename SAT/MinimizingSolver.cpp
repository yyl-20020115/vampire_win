...
SATSolver::VarAssignment MinimizingSolver::getAssignment(unsigned var)
{
  ...
  return _asgn[var] ? SATSolver::VarAssignment::_TRUE : SATSolver::VarAssignment::_FALSE;
}
...
void MinimizingSolver::processInnerAssignmentChanges()
{
  ...
  for(unsigned v=1; v<=_varCnt; v++) {
    ...
    case VarAssignment::DONT_CARE:
      changed = false;
      break;
    case VarAssignment::_TRUE:
      changed = !_asgn[v];
      _asgn[v] = true;
      break;
    case VarAssignment::_FALSE:
      changed = _asgn[v];
      _asgn[v] = false;
      break;
...
void MinimizingSolver::updateAssignment()
{
  ...
  TimeCounter tca(Lib::TimeCounterUnit::TC_MINIMIZING_SOLVER);
