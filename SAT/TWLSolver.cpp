...
: _generateProofs(generateProofs), _status(Status::SATISFIABLE), _assignment(0), _assignmentLevels(0),
...
void TWLSolver::ensureVarCount(unsigned newVarCnt)
{
  ...
  _assignment.expand(newVarCnt+1,(int)SAT::TWLSolver::AsgnVal::AS_UNDEFINED);
  _assignmentLevels.expand(newVarCnt+1);
  _assignmentPremises.expand(newVarCnt+1, 0);
  _lastAssignments.expand(newVarCnt+1, (int)SAT::TWLSolver::AsgnVal::AS_UNDEFINED);
...
unsigned TWLSolver::newVar()
{
  ...
  _assignment.expand(_varCnt+1, (unsigned)SAT::TWLSolver::AsgnVal::AS_UNDEFINED);
  _assignmentLevels.expand(_varCnt+1);
  _assignmentPremises.expand(_varCnt+1, 0);
  _lastAssignments.expand(_varCnt+1, (unsigned)SAT::TWLSolver::AsgnVal::AS_UNDEFINED);
...
void TWLSolver::addClause(SATClause* cl)
{
  ...
  TimeCounter tc(Lib::TimeCounterUnit::TC_TWLSOLVER_ADD);
...
const SATLiteralStack& TWLSolver::explicitlyMinimizedFailedAssumptions(unsigned conflictCountLimit, bool randomize)
{
  ...
  ALWAYS(solve(conflictCountLimit) == Status::UNSATISFIABLE);
  retractAllAssumptions();
...
void TWLSolver::addAssumption(SATLiteral lit)
{
  ...
  if (solve(0u) == Status::UNSATISFIABLE) {
    return;
  }
...
void TWLSolver::doSolving(unsigned conflictNumberLimit)
{
  ...
  if(conflictNumberLimit!=0) {
    ...
    if(slRes== SAT::TWLSolver::SatLoopResult::SLR_SATISFIABLE) {
...
TWLSolver::ClauseVisitResult TWLSolver::visitWatchedClause(Watch watch, unsigned var, unsigned& litIndex)
{
  ...
  if(isTrue(watch.blocker)) {
    return SAT::TWLSolver::ClauseVisitResult::VR_NONE;
  }
...
//  if(isTrue(otherWatched)) {
    return SAT::TWLSolver::ClauseVisitResult::VR_NONE; //the other watched literal is true
  }
  ...
  return SAT::TWLSolver::ClauseVisitResult::VR_PROPAGATE;
}
...
//  WatchStack::Iterator wit(getTriggeredWatchStack(var, _assignment[var]));
  ...
  while(wit.hasNext()) {
    ...
    switch(cvr) {
    case SAT::TWLSolver::ClauseVisitResult::VR_CHANGE_WATCH:
    {
    ...
    case SAT::TWLSolver::ClauseVisitResult::VR_CONFLICT:
      return cl;
    case SAT::TWLSolver::ClauseVisitResult::VR_PROPAGATE:
    {
    ...
    case SAT::TWLSolver::ClauseVisitResult::VR_NONE:
      break;
    }
  }
  return 0;
...
void TWLSolver::setAssignment(unsigned var, unsigned polarity)
{
  ...
  _assignment[var] = (int)static_cast<AsgnVal>(polarity);
  _lastAssignments[var] = (int)static_cast<AsgnVal>(polarity);
}
...
void TWLSolver::undoAssignment(unsigned var)
{
  ...
  _assignment[var] = (int)SAT::TWLSolver::AsgnVal::AS_UNDEFINED;
  _assignmentPremises[var] = 0;
...
void TWLSolver::assertValid()
{
  ...
  for(unsigned i=1;i<=_varCnt;i++) {
    if(_assignment[i]!=(int)SAT::TWLSolver::AsgnVal::AS_UNDEFINED) {
  ...
  while(uit.hasNext()) {
    USRec rec=uit.next();
    ASS_NEQ(_assignment[rec.var], (int)SAT::TWLSolver::AsgnVal::AS_UNDEFINED);
  }
}
...
inline WatchStack& TWLSolver::getTriggeredWatchStack(unsigned var, PackedAsgnVal assignment)
{
  ...
  ASS_G(var,0); ASS_LE(var,_varCnt);
  ASS(assignment!= (int)AsgnVal::AS_UNDEFINED);
...
inline bool TWLSolver::isTrue(const SATLiteral& lit) const
{
  ...
  return _assignment[lit.var()] == (int)static_cast<AsgnVal>(lit.polarity());
}
...
inline bool TWLSolver::isFalse(const SATLiteral& lit) const
{
  ...
  return _assignment[lit.var()] == (int)static_cast<AsgnVal>(lit.oppositePolarity());
}
...
SATSolver::VarAssignment TWLSolver::getAssignment(unsigned var)
{
  ...
  ASS_G(var,0); ASS_LE(var,_varCnt);
  ASS(_status == Status::SATISFIABLE || _status == Status::UNKNOWN);
          
  if(isTrue(var)) {
    return SATSolver::VarAssignment::_TRUE;
  }
    ...
    */
    return SATSolver::VarAssignment::DONT_CARE;
  }
  else {
    return SATSolver::VarAssignment::_FALSE;
  }
}
...
void TWLSolver::collectZeroImplied(SATLiteralStack& acc)
{
  ...
  ASS_NEQ(_status,Status::UNSATISFIABLE);
...
SATClause* TWLSolver::getZeroImpliedCertificate(unsigned var)
{
  ...
  for(;;) {
    ...
    while(toDo.isNonEmpty() && !_assignmentPremises[toDo.top()]) {
      ...
      ASS_NEQ(_assignment[currVar], (int)AsgnVal::AS_UNDEFINED);
...
void TWLSolver::printAssignment()
{
  ...
  for(unsigned i=1;i<=_varCnt;i++) {
    if(_assignment[i]== (int)AsgnVal::AS_UNDEFINED) {
      cout<<i<<"\t"<<(int)static_cast<AsgnVal>(_assignment[i])<<endl;
    } else {
      cout<<i<<"\t"<< (int)static_cast<AsgnVal>(_assignment[i])<<"\t"<<getAssignmentLevel(i);
...
TWLSolver::SatLoopResult TWLSolver::runSatLoop(unsigned conflictCountLimit)
{
  ...
  SatLoopResult res = SAT::TWLSolver::SatLoopResult::SLR_UNKNOWN;
  ...
  for(;;) {
	...
	res = SAT::TWLSolver::SatLoopResult::SLR_CONFLICT_LIMIT_REACHED;
	break;
      }
      unsigned choiceVar;
	...
	res = SAT::TWLSolver::SatLoopResult::SLR_SATISFIABLE;
	break;
      }
      PackedAsgnVal asgn = _lastAssignments[choiceVar];
      if(asgn==(unsigned)SAT::TWLSolver::AsgnVal::AS_UNDEFINED) {
//	asgn = (getWatchStack(choiceVar, 0).size()>getWatchStack(choiceVar, 1).size()) ? AS_FALSE : AS_TRUE;
	asgn = (PackedAsgnVal)((getWatchStack(choiceVar, 0).size()>getWatchStack(choiceVar, 1).size()) ? SAT::TWLSolver::AsgnVal::AS_TRUE : SAT::TWLSolver::AsgnVal::AS_FALSE);
      }
  ...
  ASS_NEQ(res, SAT::TWLSolver::SatLoopResult::SLR_UNKNOWN);
  return res;
}
