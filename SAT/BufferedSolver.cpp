...
SATSolver::VarAssignment BufferedSolver::getAssignment(unsigned var)
{
  ...
  if(!_literalBuffer.isEmpty() && _literalBuffer.find(var)) {
    return _literalBuffer.get(var) ? SATSolver::VarAssignment::_TRUE : SATSolver::VarAssignment::_FALSE;
  }
  ...
  if (var <= _varCntInnerOld) {
    ...
    return SATSolver::VarAssignment::DONT_CARE; // If it is new and not yet assigned in buffer
  }
}
