...
Problem* getPreprocessedProblem()
{
  ...
  TimeCounter tc2(Lib::TimeCounterUnit::TC_PREPROCESSING);
...
void boundPropagationMode(){
  ...
  try
  {
    ...
    TimeCounter tc(Lib::TimeCounterUnit::TC_SOLVING);
...
static SATClauseList* getInputClauses(const char* fname, unsigned& varCnt)
{
  ...
  TimeCounter tc(Lib::TimeCounterUnit::TC_PARSING);
...
static SATClauseIterator preprocessClauses(SATClauseList* clauses) {
  ...
  TimeCounter tc(Lib::TimeCounterUnit::TC_PREPROCESSING);
...
{
  CALL("satSolverMode()");
  TimeCounter tc(Lib::TimeCounterUnit::TC_SAT_SOLVER);
  SATSolverSCP solver;
  ...
  env.statistics->phase = Statistics::ExecutionPhase::FINALIZATION;

  switch(res) {
  case SATSolver::Status::SATISFIABLE:
    cout<<"SATISFIABLE\n";
    env.statistics->terminationReason = Statistics::TerminationReason::SAT_SATISFIABLE;
    break;
  case SATSolver::Status::UNSATISFIABLE:
    cout<<"UNSATISFIABLE\n";
    env.statistics->terminationReason = Statistics::TerminationReason::SAT_UNSATISFIABLE;
    break;
  ...
  env.endOutput();
  if (env.statistics->terminationReason == Statistics::TerminationReason::SAT_UNSATISFIABLE
      || env.statistics->terminationReason == Statistics::TerminationReason::SAT_SATISFIABLE) {
      vampireReturnValue = VAMP_RESULT_STATUS_SUCCESS;
  }
...
void vampireMode()
{
  ...
  if (env.statistics->terminationReason == Statistics::TerminationReason::REFUTATION
      || env.statistics->terminationReason == Statistics::TerminationReason::SATISFIABLE) {
      vampireReturnValue = VAMP_RESULT_STATUS_SUCCESS;
  }
} // vampireMode
