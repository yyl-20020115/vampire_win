...
public:
  ...
  T popWithAllEqual()
  {
    CALL("BinaryHeap::pop");
    T res=pop();
    while(!isEmpty() && Comparator::compare(res, top())== Lib::Comparison::EQUAL) {
      pop();
    }
    return res;
  }
...
private:
  ...
  unsigned bubbleUp(unsigned index)
  {
    ...
    while(nextIndex) {
      if(Comparator::compare(_data1[index], _data1[nextIndex])==Comparison::LESS) {
  ...
  unsigned bubbleDown(unsigned index)
  {
    ...
    while(nextIndex<=_size) {
      if(nextIndex!=_size && Comparator::compare(_data1[index], _data1[nextIndex|1])== Lib::Comparison::GREATER) {
	if(Comparator::compare(_data1[nextIndex|1], _data1[nextIndex])== Lib::Comparison::GREATER) {
	  std::swap(_data1[index], _data1[nextIndex]);
	} else {
	  std::swap(_data1[index], _data1[nextIndex|1]);
	  nextIndex|=1;
	}
      } else if(Comparator::compare(_data1[index], _data1[(int)nextIndex])== Lib::Comparison::GREATER) {
	std::swap(_data1[index], _data1[nextIndex]);
      } else {
	return index;
      }
