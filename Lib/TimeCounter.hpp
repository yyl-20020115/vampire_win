...
public:
  ...
  void stop()
  {
    if(!s_measuring) return;
    stopMeasuring();
    _tcu=Lib::TimeCounterUnit::__TC_NONE; //so that nothing happens when we call stopMeasuring() in destructor
  }

  static bool isBeingMeasured(TimeCounterUnit tcu)
  {
    return s_measureInitTimes[(int)tcu]!=-1;
  }
