...
public:
  ...
  Value* insertPosition(Key key)
  {
	...
	case Comparison::EQUAL: //we insert equal elements next to each other
	case Comparison::GREATER:
	  left = next;
	  break;
  ...
  bool findLeastGreater(Key key, Value& value)
  {
   ...
   // left is a node with a value smaller than that of newNode and having
	...
	case Comparison::EQUAL:
	  while(next->nodes[0]) {
	    next=next->nodes[0];
	    if(ValueComparator::compare(key,next->value)== Comparison::LESS) {
	      value=next->value;
	      return true;
	    }
	  }
  ...
  void remove(Key key)
  {
	...
	case Comparison::EQUAL:
      ...
      if (h > 0 && found->nodes[0] && found->nodes[h] != found->nodes[0] &&
          ValueComparator::compare(key, found->nodes[0]->value) == Comparison:: EQUAL) {
	  ...
	  for(;;) {
	    ...
	    while(left->nodes[h]!=found) {
	      left=left->nodes[h];
	      ASS(ValueComparator::compare(key,left->value)!= Comparison::LESS);
	    }
	  }
