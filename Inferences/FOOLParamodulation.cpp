...
ClauseIterator FOOLParamodulation::generateClauses(Clause* premise) {
  ...
  while (literals.hasNext()) {
    ...
    while (nvi.hasNext()) {
      ...
      if (resultType == (unsigned)Sorts::DefaultSorts::SRT_BOOL) {
        booleanTerm = subterm;
        goto substitution;
      }
    }
  ...
  (*conclusion)[conclusion->length() - 1] = Literal::createEquality(true, booleanTerm, fols, (unsigned)Sorts::DefaultSorts::SRT_BOOL);
