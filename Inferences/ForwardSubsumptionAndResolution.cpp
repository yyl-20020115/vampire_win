...
void ForwardSubsumptionAndResolution::attach(SaturationAlgorithm* salg)
{
  ...
  _unitIndex=static_cast<UnitClauseLiteralIndex*>(
	  _salg->getIndexManager()->request(Indexing::IndexType::SIMPLIFYING_UNIT_CLAUSE_SUBST_TREE) );
  _fwIndex=static_cast<FwSubsSimplifyingLiteralIndex*>(
	  _salg->getIndexManager()->request(Indexing::IndexType::FW_SUBSUMPTION_SUBST_TREE) );
}
...
void ForwardSubsumptionAndResolution::detach()
{
  ...
  _unitIndex=0;
  _fwIndex=0;
  _salg->getIndexManager()->release(Indexing::IndexType::SIMPLIFYING_UNIT_CLAUSE_SUBST_TREE);
  _salg->getIndexManager()->release(Indexing::IndexType::FW_SUBSUMPTION_SUBST_TREE);
...
bool ForwardSubsumptionAndResolution::perform(Clause* cl, Clause*& replacement, ClauseIterator& premises)
{
  ...
  TimeCounter tc_fs(Lib::TimeCounterUnit::TC_FORWARD_SUBSUMPTION);
  ...
  {
    TimeCounter tc_fsr(Lib::TimeCounterUnit::TC_FORWARD_SUBSUMPTION_RESOLUTION);
