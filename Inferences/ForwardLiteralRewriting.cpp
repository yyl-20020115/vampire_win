...
void ForwardLiteralRewriting::attach(SaturationAlgorithm* salg)
{
  ...
  _index=static_cast<RewriteRuleIndex*>(
	  _salg->getIndexManager()->request(Indexing::IndexType::REWRITE_RULE_SUBST_TREE) );
}
...
void ForwardLiteralRewriting::detach()
{
  ...
  _index=0;
  _salg->getIndexManager()->release(Indexing::IndexType::REWRITE_RULE_SUBST_TREE);
...
bool ForwardLiteralRewriting::perform(Clause* cl, Clause*& replacement, ClauseIterator& premises)
{
  ...
  TimeCounter tc(Lib::TimeCounterUnit::TC_FORWARD_LITERAL_REWRITING);
  ...
  for(unsigned i=0;i<clen;i++) {
	...
	continue;
      ...
      if(ordering.compare(lit, rhsS)!=Ordering::Result::GREATER) {
	continue;
      }
