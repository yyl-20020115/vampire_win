...
void ForwardDemodulation::attach(SaturationAlgorithm* salg)
{
  ...
  _index=static_cast<DemodulationLHSIndex*>(
	  _salg->getIndexManager()->request(Indexing::IndexType::DEMODULATION_LHS_SUBST_TREE) );
...
void ForwardDemodulation::detach()
{
  ...
  _index=0;
  _salg->getIndexManager()->release(Indexing::IndexType::DEMODULATION_LHS_SUBST_TREE);
...
bool ForwardDemodulation::perform(Clause* cl, Clause*& replacement, ClauseIterator& premises)
{
  ...
  TimeCounter tc(Lib::TimeCounterUnit::TC_FORWARD_DEMODULATION);
  ...
  for(unsigned li=0;li<cLen;li++) {
	...
	bool preordered = argOrder==Ordering::Result::LESS || argOrder==Ordering::Result::GREATER;
#if VDEBUG
	if(preordered) {
	  if(argOrder==Ordering::Result::LESS) {
...
#endif
	if(!preordered && (_preorderedOnly || ordering.compare(trm,rhsS)!=Ordering::Result::GREATER) ) {
	  continue;
	}
	...
	if(toplevelCheck) {
	  ...
	  if(tord!=Ordering::Result::LESS && tord!=Ordering::Result::LESS_EQ) {
		...
		continue;
	      ...
	      if(ordering.compare(eqLitS, (*cl)[li2])==Ordering::Result::LESS) {
		isMax=false;
		break;
	      }
	    }
