      ...
      while(it.hasNext()){
        ...
        if((complexTermsAllowed || env.signature->functionArity(f)==0) &&
         ...
         if(structInd && 
          ...
          if(mathInd && 
             env.signature->getFunction(f)->fnType()->result()==(unsigned)Sorts::DefaultSorts::SRT_INTEGER &&
...
void InductionClauseIterator::performMathInductionOne(Clause* premise, Literal* origLit, Literal* lit, Term* term, InferenceRule rule) 
{
  ...
  // create L[X+1] 
  TermList fpo(Term::create2(env.signature->getInterpretingSymbol(Theory::Interpretation::INT_PLUS),x,one));
  TermReplacement cr4(term,fpo);
  Formula* Lxpo = new AtomicFormula(cr4.transform(clit));

  // create L[X-1]
  TermList fmo(Term::create2(env.signature->getInterpretingSymbol(Theory::Interpretation::INT_PLUS),x,mone));
  TermReplacement cr5(term,fmo);
  Formula* Lxmo = new AtomicFormula(cr5.transform(clit));

  // create X>=0, which is ~X<0
  Formula* Lxgz = new AtomicFormula(Literal::create2(env.signature->getInterpretingSymbol(Theory::Interpretation::INT_LESS),
                                   false,x,zero));
  // create Y>=0, which is ~Y<0
  Formula* Lygz = new AtomicFormula(Literal::create2(env.signature->getInterpretingSymbol(Theory::Interpretation::INT_LESS),
                                   false,y,zero));
  // create X<=0, which is ~0<X
  Formula* Lxlz = new AtomicFormula(Literal::create2(env.signature->getInterpretingSymbol(Theory::Interpretation::INT_LESS),
                                   false,zero,x));
  // create Y<=0, which is ~0<Y
  Formula* Lylz = new AtomicFormula(Literal::create2(env.signature->getInterpretingSymbol(Theory::Interpretation::INT_LESS),
                                   false,zero,y));
