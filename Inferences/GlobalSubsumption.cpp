...
void GlobalSubsumption::attach(SaturationAlgorithm* salg)
{
  ...
  _index=static_cast<GroundingIndex*>(
	  _salg->getIndexManager()->request(Indexing::IndexType::GLOBAL_SUBSUMPTION_INDEX) );
...
void GlobalSubsumption::detach()
{
  ...
  _index=0;
  _salg->getIndexManager()->release(Indexing::IndexType::GLOBAL_SUBSUMPTION_INDEX);
...
Clause* GlobalSubsumption::perform(Clause* cl, Stack<Unit*>& prems)
{
  ...
  TimeCounter tc(Lib::TimeCounterUnit::TC_GLOBAL_SUBSUMPTION);

  if(cl->color()== Color::COLOR_LEFT) {
    return cl;
  }
      ...
      if (survivors.size() < clen) {
        ...
        SATInference::collectFilteredFOPremises(ref, prems,
          ...
          [this,cl] (SATClause* prem) {
            ...
            if (prem->inference()->getType() == SATInference::InfType::ASSUMPTION) {
              ASS_EQ(prem->size(),1);
              return false;
            }
