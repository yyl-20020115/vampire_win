...
void BackwardDemodulation::attach(SaturationAlgorithm* salg)
{
  ...
  _index=static_cast<DemodulationSubtermIndex*>(
	  _salg->getIndexManager()->request(Indexing::IndexType::DEMODULATION_SUBTERM_SUBST_TREE) );
}
...
void BackwardDemodulation::detach()
{
  ...
  _index=0;
  _salg->getIndexManager()->release(Indexing::IndexType::DEMODULATION_SUBTERM_SUBST_TREE);
...
struct BackwardDemodulation::ResultFn
{
  ...
  OWN_RETURN_TYPE operator() (pair<TermList,TermQueryResult> arg)
  {
    ...
    if(_ordering.compare(lhsS,rhsS)!= Ordering::Result::GREATER) {
      return BwSimplificationRecord(0);
    }
    ...
    if(_parent.getOptions().demodulationRedundancyCheck() && qr.literal->isEquality() &&
      ...
      if(tord!=Ordering::Result::LESS && tord!= Ordering::Result::LESS_EQ) {
        ...
        while(cit.hasNext()) {
          ...
          if(_ordering.compare(eqLitS, lit2)== Ordering::Result::LESS) {
            isMax=false;
            break;
          }
        }
	...
	BwSimplificationRecordIterator& simplifications)
{
  ...
  TimeCounter tc(Lib::TimeCounterUnit::TC_BACKWARD_DEMODULATION);
