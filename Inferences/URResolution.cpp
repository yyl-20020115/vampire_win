...
void URResolution::attach(SaturationAlgorithm* salg)
{
  ...
  _unitIndex = static_cast<UnitClauseLiteralIndex*> (
	  _salg->getIndexManager()->request(Indexing::IndexType::GENERATING_UNIT_CLAUSE_SUBST_TREE) );
  _nonUnitIndex = static_cast<NonUnitClauseLiteralIndex*> (
	  _salg->getIndexManager()->request(Indexing::IndexType::GENERATING_NON_UNIT_CLAUSE_SUBST_TREE) );
...
void URResolution::detach()
{
  ...
  _unitIndex = 0;
  _salg->getIndexManager()->release(Indexing::IndexType::GENERATING_UNIT_CLAUSE_SUBST_TREE);
  _nonUnitIndex = 0;
  _salg->getIndexManager()->release(Indexing::IndexType::GENERATING_NON_UNIT_CLAUSE_SUBST_TREE);
...
//    ASS_GE(_activeLength, clen-1);
  ...
  void resolveLiteral(unsigned idx, SLQueryResult& unif, Clause* premise, bool useQuerySubstitution)
  {
    ...
    _color = static_cast<Color>((unsigned)_color | (unsigned)premise->color());
    ASS_NEQ(_color, Color::COLOR_INVALID)
...
ClauseIterator URResolution::generateClauses(Clause* cl)
{
  ...
  TimeCounter tc(Lib::TimeCounterUnit::TC_UR_RESOLUTION);
