    ...
    while(it.hasNext()){
      ...
      if(t.isTerm() && t.term()->ground()){
        ...
        if(SortHelper::tryGetResultSort(t,sort)){
          if(sort==(unsigned)Sorts::DefaultSorts::SRT_DEFAULT) continue;
...
void Instantiation::tryMakeLiteralFalse(Literal* lit, Stack<Substitution>& subs)
{
  ...
  if(theory->isInterpretedPredicate(lit)){
    ...
    if (itp == Theory::Interpretation::EQUAL || itp == Theory::Interpretation::INT_LESS || itp == Theory::Interpretation::RAT_LESS || itp == Theory::Interpretation::REAL_LESS) {
