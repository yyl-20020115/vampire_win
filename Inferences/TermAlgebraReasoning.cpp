...
namespace Inferences {
    ...
    if (sameConstructorsEquality(lit) && !lit->polarity()) {
      ...
      for (unsigned j = 0; j < arity; j++) {
        ...
        for (unsigned k = 0; k < c->length(); k++) {
          if (k != i) {
            if (_salg->getOrdering().compare((*c)[k], l) != Ordering::Result::GREATER) {
              return false;
            }
          }
        }
  ...
  void AcyclicityGIE::attach(SaturationAlgorithm* salg)
  {
    ...
    _acyclIndex = static_cast<AcyclicityIndex*>(_salg->getIndexManager()->request(Indexing::IndexType::ACYCLICITY_INDEX));
  }
  ...
  void AcyclicityGIE::detach()
  {
    ...
    _acyclIndex = 0;
    _salg->getIndexManager()->release(Indexing::IndexType::ACYCLICITY_INDEX);
