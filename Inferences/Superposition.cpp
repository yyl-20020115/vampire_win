...
void Superposition::attach(SaturationAlgorithm* salg)
{
  ...
  _subtermIndex=static_cast<SuperpositionSubtermIndex*> (
	  _salg->getIndexManager()->request(Indexing::IndexType::SUPERPOSITION_SUBTERM_SUBST_TREE) );
  _lhsIndex=static_cast<SuperpositionLHSIndex*> (
	  _salg->getIndexManager()->request(Indexing::IndexType::SUPERPOSITION_LHS_SUBST_TREE) );
}
...
void Superposition::detach()
{
  ...
  _subtermIndex=0;
  _lhsIndex=0;
  _salg->getIndexManager()->release(Indexing::IndexType::SUPERPOSITION_SUBTERM_SUBST_TREE);
  _salg->getIndexManager()->release(Indexing::IndexType::SUPERPOSITION_LHS_SUBST_TREE);
...
ClauseIterator Superposition::generateClauses(Clause* premise)
{
  ...
  auto it7 = getTimeCountedIterator(it6, Lib::TimeCounterUnit::TC_SUPERPOSITION);
    ...
    UnificationConstraintStackSP constraints)
{
  ...
  ASS(rwClause->store()== Clause::Store::ACTIVE);
  ASS(eqClause->store()== Clause::Store::ACTIVE);
...
*/
  ...
  for(unsigned i=0;i<rwLength;i++) {
    ...
    if(curr!=rwLit) {
      ...
      if (afterCheck) {
        TimeCounter tc(Lib::TimeCounterUnit::TC_LITERAL_ORDER_AFTERCHECK);
        if (i < rwClause->numSelected() && ordering.compare(currAfter,rwLitS) == Ordering::Result::GREATER) {
    ...
    for(unsigned i=0;i<eqLength;i++) {
      ...
      if(curr!=eqLit) {
        ...
        if (eqLitS && i < eqClause->numSelected()) {
          ...
          if (o == Ordering::Result::GREATER || o == Ordering::Result::GREATER_EQ || o == Ordering::Result::EQUAL) { // where is GREATER_EQ ever coming from?
            env.statistics->inferencesBlockedForOrderingAftercheck++;
            goto construction_fail;
          }
        }
