                                               ...
                                               ClauseList*& newClauses)
{
    ...
    Formula* injective = 
      new QuantifiedFormula(Connective::FORALL,xy,0,new BinaryFormula(Connective::IMP,eq_fxfy,eq_xy));

    Formula* surjective =
      new QuantifiedFormula(Connective::FORALL, new Formula::VarList(1),0,
      new QuantifiedFormula(Connective::EXISTS, new Formula::VarList(0),0,
      new AtomicFormula(Literal::createEquality(true,fx,y,ret_srt))));

    Formula* ing_and_nons = new JunctionFormula(Connective::AND,
                            new FormulaList(injective, new FormulaList(new NegatedFormula(surjective))));
    Formula* sur_and_noni = new JunctionFormula(Connective::AND,
                            new FormulaList(surjective, new FormulaList(new NegatedFormula(injective))));

    if(existential){
      injective  = new QuantifiedFormula(Connective::EXISTS, existential, 0, injective);
      surjective = new QuantifiedFormula(Connective::EXISTS, existential, 0, surjective);
      ing_and_nons = new QuantifiedFormula(Connective::EXISTS, existential, 0, ing_and_nons);
      sur_and_noni = new QuantifiedFormula(Connective::EXISTS, existential, 0, sur_and_noni);
    }
    // Add names (true/false relates to being injective or not i.e. surjective)
    injective    = new BinaryFormula(Connective::IMP,injective,getName(ret_srt,arg_srt,false));
    surjective   = new BinaryFormula(Connective::IMP,surjective,getName(arg_srt,ret_srt,false));
    ing_and_nons = new BinaryFormula(Connective::IMP,ing_and_nons,getName(ret_srt,arg_srt,true));
    sur_and_noni = new BinaryFormula(Connective::IMP,sur_and_noni,getName(arg_srt,ret_srt,true));
