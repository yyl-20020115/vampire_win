...
void FiniteModelBuilder::init()
{
  ...
  env.statistics->phase = Statistics::ExecutionPhase::FMB_PREPROCESSING;
  ...
  {
    TimeCounter tc(Lib::TimeCounterUnit::TC_FMB_SPLITTING);
  ...
  {
    TimeCounter tc(Lib::TimeCounterUnit::TC_FMB_SORT_INFERENCE);
  ...
  cout << "Finding Min and Max Sort Sizes" << endl;
    ...
    for(unsigned s=0;s<_sortedSignature->distinctSorts;s++){
      bool epr = env.property->category()==Property::Category::EPR
    ...
    for(unsigned s=0;s<env.sorts->count();s++){
      if((env.property->usesSort(s) || s >= (unsigned)Sorts::DefaultSorts::FIRST_USER_SORT) && _sortedSignature->vampireToDistinct.find(s)){
  ...
  for(unsigned f=0;f<env.signature->functions();f++){
    ...
    if(env.signature->functionArity(f)==0){ 
      ...
      if(vsrt != (unsigned)Sorts::DefaultSorts::SRT_BOOL){
...
MainLoopResult FiniteModelBuilder::runImpl()
{
  ...
  if(!_isAppropriate){
    // give up!
    return MainLoopResult(Statistics::TerminationReason::INAPPROPRIATE);
  }
  if(!_prb.units()){
    return MainLoopResult(Statistics::TerminationReason::SATISFIABLE);
  }

  env.statistics->phase = Statistics::ExecutionPhase::FMB_CONSTRAINT_GEN;
  ...
  while(true){
    ...
    Timer::syncClock();
    if(env.timeLimitReached()){ return MainLoopResult(Statistics::TerminationReason::TIME_LIMIT); }

    {
    TimeCounter tc(Lib::TimeCounterUnit::TC_FMB_CONSTRAINT_CREATION);
    ...
    {
      TimeCounter tc(Lib::TimeCounterUnit::TC_FMB_SAT_SOLVING);
    ...
    {
      env.statistics->phase = Statistics::ExecutionPhase::FMB_SOLVING;
      TimeCounter tc(Lib::TimeCounterUnit::TC_FMB_SAT_SOLVING);
...

    {
      ...
      if (_xmass) {
        ...
        if (domsWeight < UINT_MAX) {
          ...
          return MainLoopResult(Statistics::TerminationReason::REFUTATION,empty);
        }
      } else { // i.e. (!_xmass)
        ...
        for (unsigned i = 0; i < _distinctSortSizes.size(); i++) {
          nogood[i] = make_pair(FMB::FiniteModelBuilder::ConstraintSign::STAR,_distinctSortSizes[i]);
        }
        ...
        for (unsigned i = 0; i < failed.size(); i++) {
          ...
          if (var < instancesMarker_offset) { // totality used (-> instances used as well / unless the sort is monotonic)
            ...
            if (_sortedSignature->monotonicSorts[dsort]) {
              nogood[dsort].first = FMB::FiniteModelBuilder::ConstraintSign::LEQ;
            } else {
              nogood[dsort].first = FMB::FiniteModelBuilder::ConstraintSign::EQ;
            }
          } else if (nogood[var-instancesMarker_offset].first == FMB::FiniteModelBuilder::ConstraintSign::STAR) { // instances used (and we don't know yet about totality)
            ASS(!_sortedSignature->monotonicSorts[var-instancesMarker_offset]);
            nogood[var-instancesMarker_offset].first = FMB::FiniteModelBuilder::ConstraintSign::GEQ;
          }
        }
  ...
  */
  ...
  return MainLoopResult(Statistics::TerminationReason::REFUTATION_NOT_FOUND);
}
...
bool FiniteModelBuilder::HackyDSAE::checkConstriant(DArray<unsigned>& newSortSizes, Constraint_Generator_Vals& constraint)
{
  ...
  for (unsigned j = 0; j < newSortSizes.size(); j++) {
    ...
    if (cc.first == FMB::FiniteModelBuilder::ConstraintSign::EQ && cc.second != newSortSizes[j]) {
      return false;
    }
    if (cc.first == FMB::FiniteModelBuilder::ConstraintSign::GEQ && cc.second > newSortSizes[j]) {
      return false;
    }
    if (cc.first == FMB::FiniteModelBuilder::ConstraintSign::LEQ && cc.second < newSortSizes[j]) {
      return false;
    }
  }
      ...
      // test 3 -- (strict)_distinct_sort_constraints
      {
        ...
        while (it1.hasNext()) {
          ...
          if (newSortSizes[constr.first] < newSortSizes[constr.second]) {
            ...
            Constraint_Generator_Vals& gen = gen_p->_vals;
            for (unsigned j = 0; j < newSortSizes.size(); j++) {
              gen[j] = make_pair(FMB::FiniteModelBuilder::ConstraintSign::STAR,newSortSizes[j]);
            }
            gen[constr.first].first = FMB::FiniteModelBuilder::ConstraintSign::EQ;
            gen[constr.second].first = FMB::FiniteModelBuilder::ConstraintSign::GEQ;
...
bool FiniteModelBuilder::SmtBasedDSAE::increaseModelSizes(DArray<unsigned>& newSortSizes, DArray<unsigned>& sortMaxes)
{
  ...
  try {
    TimeCounter tc(Lib::TimeCounterUnit::TC_Z3_IN_FMB);
