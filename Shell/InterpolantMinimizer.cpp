...
Formula* InterpolantMinimizer::getInterpolant(Unit* refutation)
{
  ...
  if(mres==SMTSolver::MinimizationResult::FAIL) {
  ...
  if(mres==SMTSolver::MinimizationResult::APPROXIMATE) {
...
void InterpolantMinimizer::prettyPrint(Formula* formula, ostream& out)
{
  ...
  case Kernel::Connective::LITERAL:
        ...
        return;
    case Kernel::Connective::AND:
    case Kernel::Connective::OR:
        if(formula->connective() == Kernel::Connective::AND)
            out << "(and ";
        else
            out << "(or ";
        ...
        out << ")";
        return;
    case Kernel::Connective::IMP:
    case Kernel::Connective::IFF:
    case Kernel::Connective::XOR:
        if(formula->connective() == Kernel::Connective::IMP)
                out << "(implies ";
        else if(formula->connective() == Kernel::Connective::IFF)
            out << "(= ";
        else
            ASS(false);
        ...
        out << ")";
        return;
    case Kernel::Connective::NOT:
        out << "(not ";
        prettyPrint(formula->uarg(), out);
        out << ")";
        return;
    case Kernel::Connective::FORALL:
        return;
    case Kernel::Connective::EXISTS:
        return;
    case Kernel::Connective::BOOL_TERM:
        out << formula->getBooleanTerm().toString();
        return;
    case Kernel::Connective::_TRUE:
        out << "true";
        return;
    case Kernel::Connective::_FALSE:
        out << "false";
        return;
    default:
...
void InterpolantMinimizer::collectSlicedOffNodes(SMTSolverResult& solverResult, DHSet<Unit*>& acc)
{
  ...
  while(uit.hasNext()) {
    ...
    SMTConstant sU = pred(Shell::InterpolantMinimizer::PredType::S, uid);
...
void InterpolantMinimizer::addNodeFormulas(Unit* u)
{
  ...
  if(_noSlicing || uinfo.isRefutation) {
    ...
    _resBenchmark.addFormula(!pred(Shell::InterpolantMinimizer::PredType::S,uId), comment);
  }

  //if formula is a parent of colored formula, we do not allow to have
  //opposite color in the trace.
  if(uinfo.isParentOfLeft) {
    _resBenchmark.addFormula(!pred(Shell::InterpolantMinimizer::PredType::B,uId), "parent_of_left");
  }
  if(uinfo.isParentOfRight) {
    _resBenchmark.addFormula(!pred(Shell::InterpolantMinimizer::PredType::R,uId), "parent_of_right");
  }
...
void InterpolantMinimizer::addFringeFormulas(Unit* u)
{
  ...
  SMTFormula rcN = pred(Shell::InterpolantMinimizer::PredType::RC, n);
  SMTFormula bcN = pred(Shell::InterpolantMinimizer::PredType::BC, n);
  SMTFormula rfN = pred(Shell::InterpolantMinimizer::PredType::RF, n);
  SMTFormula bfN = pred(Shell::InterpolantMinimizer::PredType::BF, n);
  SMTFormula dN = pred(Shell::InterpolantMinimizer::PredType::D, n);
  ...
  while(gsit.hasNext()) {
    ...
    SMTFormula rcS = pred(Shell::InterpolantMinimizer::PredType::RC, succId);
    SMTFormula bcS = pred(Shell::InterpolantMinimizer::PredType::BC, succId);
    SMTFormula rfS = pred(Shell::InterpolantMinimizer::PredType::RF, succId);
    SMTFormula bfS = pred(Shell::InterpolantMinimizer::PredType::BF, succId);
...
void InterpolantMinimizer::addAtomImplicationFormula(Unit* u)
{
  ...
  while(ait.hasNext()) {
    ...
    cConj = cConj & pred(Shell::InterpolantMinimizer::PredType::V, atom);
  }

  vstring comment = "atom implications for " + u->toString();
  _resBenchmark.addFormula(pred(Shell::InterpolantMinimizer::PredType::D, uId) --> cConj, comment);
}
...
void InterpolantMinimizer::addCostFormula()
{
  ...
  while(wit.hasNext()) {
    ...
    if(_optTarget==Shell::InterpolantMinimizer::OptimizationTarget::OT_COUNT && weight>0) {
      weight = 1;
    }
    //**minimize the interpolant wrt the number of quantifiers
    if(_optTarget==Shell::InterpolantMinimizer::OptimizationTarget::OT_QUANTIFIERS){
      weight = varCnt;
    }

    TRACE(cout << "cost " << weight << " for unit " << unit->toString() << endl);

    SMTFormula atomExpr = SMTFormula::condNumber(pred(Shell::InterpolantMinimizer::PredType::V, atom), weight);
...
SMTConstant InterpolantMinimizer::pred(PredType t, vstring node)
{
  ...
  vstring n1;
  switch(t) {
  case Shell::InterpolantMinimizer::PredType::R: n1 = "r"; break;
  case Shell::InterpolantMinimizer::PredType::B: n1 = "b"; break;
  case Shell::InterpolantMinimizer::PredType::G: n1 = "g"; break;
  case Shell::InterpolantMinimizer::PredType::S: n1 = "s"; break;
  case Shell::InterpolantMinimizer::PredType::RC: n1 = "rc"; break;
  case Shell::InterpolantMinimizer::PredType::BC: n1 = "bc"; break;
  case Shell::InterpolantMinimizer::PredType::RF: n1 = "rf"; break;
  case Shell::InterpolantMinimizer::PredType::BF: n1 = "bf"; break;
  case Shell::InterpolantMinimizer::PredType::D: n1 = "d"; break;
  case Shell::InterpolantMinimizer::PredType::V: n1 = "v"; break;
  default: ASSERTION_VIOLATION;
  }
...
void InterpolantMinimizer::addDistinctColorsFormula(vstring n)
{
  ...
  SMTFormula rN = pred(Shell::InterpolantMinimizer::PredType::R, n);
  SMTFormula bN = pred(Shell::InterpolantMinimizer::PredType::B, n);
  SMTFormula gN = pred(Shell::InterpolantMinimizer::PredType::G, n);
...
void InterpolantMinimizer::addGreyNodePropertiesFormula(vstring n, ParentSummary& parents)
{
  ...
  while(pit.hasNext()) {
    ...
    rParDisj = rParDisj | pred(Shell::InterpolantMinimizer::PredType::R, par);
    bParDisj = bParDisj | pred(Shell::InterpolantMinimizer::PredType::B, par);
    gParConj = gParConj & pred(Shell::InterpolantMinimizer::PredType::G, par);
  }

  SMTFormula rN = pred(Shell::InterpolantMinimizer::PredType::R, n);
  SMTFormula bN = pred(Shell::InterpolantMinimizer::PredType::B, n);
  SMTFormula gN = pred(Shell::InterpolantMinimizer::PredType::G, n);
  SMTFormula sN = pred(Shell::InterpolantMinimizer::PredType::S, n);
  SMTFormula rcN = pred(Shell::InterpolantMinimizer::PredType::RC, n);
  SMTFormula bcN = pred(Shell::InterpolantMinimizer::PredType::BC, n);
...
void InterpolantMinimizer::addColoredParentPropertiesFormulas(vstring n, ParentSummary& parents)
{
  ...
  PredType parentType = parents.rParents.isNonEmpty() ? Shell::InterpolantMinimizer::PredType::R : Shell::InterpolantMinimizer::PredType::B;
  PredType oppositeType = (parentType== Shell::InterpolantMinimizer::PredType::R) ? Shell::InterpolantMinimizer::PredType::B : Shell::InterpolantMinimizer::PredType::R;
...
//  SMTFormula dN = pred(Shell::InterpolantMinimizer::PredType::D, n);
  ...
  if(parentType== Shell::InterpolantMinimizer::PredType::R) {
    _resBenchmark.addFormula(rcN -=- !sN);
    _resBenchmark.addFormula(!bcN);
  }
  else {
    ASS_EQ(parentType, Shell::InterpolantMinimizer::PredType::B);
...
//    Color pcol = parent.unit()->getColor();
    ...
    if(pcol==COLOR_LEFT) {
      ASS_NEQ(info.state, UnitState::HAS_RIGHT_PARENT);
      info.state = UnitState::HAS_LEFT_PARENT;
    }
    if(pcol==COLOR_RIGHT) {
      ASS_REP2(info.state!=UnitState::HAS_LEFT_PARENT, unit->toString(), parent->toString());
      info.state = UnitState::HAS_RIGHT_PARENT;
    }
