...
SMTSolver::MinimizationResult SMTSolver::minimize(SMTBenchmark& problem, SMTConstant costFn, SMTSolverResult& res)
{
  ...
  unsigned left = 1;
  unsigned guess = left;
  while(tryUpperBound(problem, costFn, guess, res)!=SMTSolverResult::Status::SAT) {
    approx |= res.status==SMTSolverResult::Status::UNKNOWN;
    unsigned newGuess = guess*2;
    if(newGuess<=guess) {
      //we had an overflow
      cerr << "cost overflow durint SMT minimization" << endl;
      return MinimizationResult::FAIL;
    }
    left = guess;
    guess = newGuess;
  }
  ...
  while(left!=right) {
    ...
    if(tryUpperBound(problem, costFn, middle, res)==SMTSolverResult::Status::SAT) {
      right = middle;
    }
    else {
      approx |= res.status== SMTSolverResult::Status::UNKNOWN;
  ...
  if(res.status== SMTSolverResult::Status::UNKNOWN) {
    //we don't even have a satisfying assignment
    return MinimizationResult::FAIL;
  }
  return approx ? MinimizationResult::APPROXIMATE : MinimizationResult::EXACT;
}
...
void YicesSolver::run(SMTBenchmark& problem, SMTSolverResult& res, unsigned timeout)
{
  ...
  while(oit.hasNext()) {
    ...
    if(line=="") { continue; }
    if(line=="sat") { res.status = SMTSolverResult::Status::SAT; continue; }
    if(line=="unsat") { res.status = SMTSolverResult::Status::UNSAT; continue; }
    if(line=="unknown") { res.status = SMTSolverResult::Status::UNKNOWN; continue; }
