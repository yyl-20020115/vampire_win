...
Formula* FOOLElimination::process(Formula* formula) {
  ...
  switch (formula->connective()) {
    case Connective::LITERAL: {
      ...
      if (literal->isEquality()) {
        ...
        if (rhsIsFormula || lhsIsFormula) {
          ...
          Connective connective = literal->polarity() ? Connective::IFF : Connective::XOR;
    ...
    case Connective::XOR: {
      ...
      if (lhs->connective() == Connective::BOOL_TERM && rhs->connective() == Connective::BOOL_TERM) {
        TermList lhsTerm = lhs->getBooleanTerm();
        TermList rhsTerm = rhs->getBooleanTerm();

        bool polarity = formula->connective() == Connective::IFF;

        Literal* equality = Literal::createEquality(polarity, process(lhsTerm), process(rhsTerm), (unsigned)Sorts::DefaultSorts::SRT_BOOL);
...
void FOOLElimination::process(TermList ts, Context context, TermList& termResult, Formula*& formulaResult) {
  ...
  if (context == FORMULA_CONTEXT) {
    ASS_REP(sort == (unsigned)Sorts::DefaultSorts::SRT_BOOL, ts.toString());
  }
#endif
  ...
  if (!term->isSpecial()) {
    ...
    switch (term->functor()) {
      case Term::SF_ITE: {
        ...
        // build (f => g(X1, ..., Xn) == s)
        Formula* thenImplication = new BinaryFormula(Connective::IMP, condition, thenEq);

        // build ![X1, ..., Xn]: (f => g(X1, ..., Xn) == s)
        if (Formula::VarList::length(freeVars) > 0) {
          //TODO do we know the sorts of freeVars?
          thenImplication = new QuantifiedFormula(Connective::FORALL, freeVars,0, thenImplication);
        }
        ...
        // build ~f => g(X1, ..., Xn) == t
        Formula* elseImplication = new BinaryFormula(Connective::IMP, new NegatedFormula(condition), elseEq);

        // build ![X1, ..., Xn]: (~f => g(X1, ..., Xn) == t)
        if (Formula::VarList::length(freeVars) > 0) {
          //TODO do we know the sorts of freeVars?
          elseImplication = new QuantifiedFormula(Connective::FORALL, freeVars, 0, elseImplication);
        }
      ...
      case Term::SF_LET: {
        ...
        */ 
          freshSymbolDefinition = new QuantifiedFormula(Connective::FORALL, vars, 0, freshSymbolDefinition);
        }
      ...
      case Term::SF_FORMULA: {
        ...
        if (connective == Connective::_TRUE) {
          termResult = TermList(Term::foolTrue());
          break;
        }

        if (connective == Connective::_FALSE) {
        ...
        unsigned freshSymbol = introduceFreshSymbol(context, BOOL_PREFIX, freeVarsSorts, (unsigned)Sorts::DefaultSorts::SRT_BOOL);
        TermList freshSymbolApplication = buildFunctionApplication(freshSymbol, freeVars);

        // build f <=> g(X1, ..., Xn) = true
        Formula* freshSymbolDefinition = new BinaryFormula(Connective::IFF, formula, toEquality(freshSymbolApplication));

        // build ![X1, ..., Xn]: (f <=> g(X1, ..., Xn) = true)
        if (Formula::VarList::length(freeVars) > 0) {
          // TODO do we know the sorts of freeVars?
          freshSymbolDefinition = new QuantifiedFormula(Connective::FORALL, freeVars,0, freshSymbolDefinition);
        }
...
Formula* FOOLElimination::buildEq(Context context, Formula* lhsFormula, Formula* rhsFormula,
  ...
  if (context == FORMULA_CONTEXT) {
    // build equivalence
    return new BinaryFormula(Connective::IFF, lhsFormula, rhsFormula);
  } else {
    // build equality
...
Formula* FOOLElimination::toEquality(TermList booleanTerm) {
  ...
  Literal* equality = Literal::createEquality(true, booleanTerm, truth, (unsigned)Sorts::DefaultSorts::SRT_BOOL);
