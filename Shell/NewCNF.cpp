                          ...
                          Stack<TermList> &thenBranches, Stack<TermList> &elseBranches)
{
  ...
  switch (sd->getType()) {
    case Term::SF_FORMULA: {
      sort = (unsigned)Sorts::DefaultSorts::SRT_BOOL;
...
void NewCNF::process(JunctionFormula *g, Occurrences &occurrences)
{
  ...
  SIGN formulaSign = g->connective() == Connective::OR ? POSITIVE : NEGATIVE;
...
void NewCNF::process(BinaryFormula* g, Occurrences &occurrences)
{
  ...
  ASS(g->connective() != Connective::IMP);

  SIGN formulaSign = g->connective() == Connective::IFF ? POSITIVE : NEGATIVE;
...
TermList NewCNF::eliminateLet(Term::SpecialTermData *sd, TermList contents)
{
  ...
  if (sd->getType() == Term::SF_LET) {
    ...
    for (unsigned proj = 0; proj < tupleType->arity(); proj++) {
      ...
      bool isPredicate = tupleType->arg(proj) == (unsigned)Sorts::DefaultSorts::SRT_BOOL;
...
TermList NewCNF::nameLetBinding(unsigned symbol, Formula::VarList* bindingVariables, TermList binding, TermList contents)
{
  ...
  if (renameSymbol) {
    ...
    while (vit.hasNext()) {
      ...
      sorts.push(_varSorts.get(var, (unsigned)Sorts::DefaultSorts::SRT_DEFAULT));
    }
...
Term* NewCNF::createSkolemTerm(unsigned var, VarSet* free)
{
  ...
  ensureHavingVarSorts();
  unsigned rangeSort=_varSorts.get(var, (unsigned)Sorts::DefaultSorts::SRT_DEFAULT);
  ...
  while(vit.hasNext()) {
    ...
    domainSorts.push(_varSorts.get(uvar, (unsigned)Sorts::DefaultSorts::SRT_DEFAULT));
    fnArgs.push(TermList(uvar, false));
  }

  Term* res;
  bool isPredicate = (rangeSort == (unsigned)Sorts::DefaultSorts::SRT_BOOL);
  if (isPredicate) {
...
void NewCNF::process(QuantifiedFormula* g, Occurrences &occurrences)
{
  ...
  while (occit.hasNext()) {
    ...
    if ((occ.sign() == POSITIVE) == (g->connective() == Connective::EXISTS)) {
...
void NewCNF::process(Formula* g, Occurrences &occurrences)
{
  ...
  case Connective::AND:
    ...
    case Connective::NOT:
      ASS_REP(g->uarg()->connective() == Connective::BOOL_TERM, g->uarg()->toString());
    ...
    case Connective::_TRUE:
    case Connective::_FALSE:
      processConstant(g->connective() == Connective::_TRUE, occurrences);
      break;
...
void NewCNF::toClauses(SPGenClause gc, Stack<Clause*>& output)
{
  ...
  while (bit.hasNext()) {
    ...
    ASS_REP(f->connective() == Connective::LITERAL, f->toString());
    skolems.push(f);
  }
...
bool NewCNF::mapSubstitution(List<GenLit>* clause, Substitution subst, bool onlyFormulaLevel, List<GenLit>* &output)
{
  ...
  while (it.hasNext()) {
    GenLit gl = it.next();
    Formula* f = (onlyFormulaLevel && (formula(gl)->connective() == Connective::LITERAL))
                  ? formula(gl)
    ...
    switch (f->connective()) {
      case Connective::_TRUE:
      case Connective::_FALSE: {
        SIGN fSign = f->connective() == Connective::_TRUE ? POSITIVE : NEGATIVE;
        if (sign(gl) == fSign) {
          return false;
        } else {
          continue;
...
Clause* NewCNF::toClause(SPGenClause gc)
{
  ...
  while (lit.hasNext()) {
    ...
    ASS_REP(g->connective() == Connective::LITERAL, gc->toString());
