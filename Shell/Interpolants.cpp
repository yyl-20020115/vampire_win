...
void Interpolants::generateInterpolant(ItemState& st)
{
  ...
  if (u->isClause()) {
    ...
    if (cl->splits()) {
      ...
      if (FormulaList::isNonEmpty(disjuncts)) {
        ...
        unitFormula = JunctionFormula::generalJunction(Connective::OR, disjuncts);
      }
    }
  }
  ...
  if(st.parCnt) {
    ...
    while(sit.hasNext()) {
      ...
      FormulaList::push(JunctionFormula::generalJunction(Connective::OR, disj), conj);
    }
    ...
    if(st.inheritedColor==COLOR_LEFT) {
      ...
      FormulaList::push(new NegatedFormula(JunctionFormula::generalJunction(Connective::AND, innerConj)), conj);
    }
    else {
      //u is justified by B or a refutation
    }
    interpolant=JunctionFormula::generalJunction(Connective::AND, conj);
  }
  else {
