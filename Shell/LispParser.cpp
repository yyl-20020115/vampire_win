...
LispParser::Expression* LispParser::parse()
{
  ...
  Expression* result = new Expression(Tag::LIST);
  parse(&result->list);
  return result;
} // parse()
...
void LispParser::parse(List** expr0)
{
  ...
  new_parsing_level:
    for (;;) {
      _lexer.readToken(t);
      switch (t.tag) {
      case Shell::TokenType::TT_RPAR:
        if (_balance == 0) {
          throw Exception("unmatched right parenthesis",t);
        }
        _balance--;
        goto parsing_level_done;
      case Shell::TokenType::TT_LPAR:
        _balance++;
        {
	  Expression* subexpr = new Expression(Tag::LIST);
	  List* sub = new List(subexpr);
	  *expr = sub;
	  expr = sub->tailPtr();
	  stack.push(expr);
	  ...
	  goto new_parsing_level;
        ...
        break;
      case Shell::TokenType::TT_NAME:
      case Shell::TokenType::TT_INTEGER:
      case Shell::TokenType::TT_REAL:
      {
        Expression* subexpr = new Expression(Tag::ATOM,t.text);
        List* sub = new List(subexpr);
        *expr = sub;
        expr = sub->tailPtr();
        break;
      }
      case Shell::TokenType::TT_EOF:
        if (_balance == 0) {
          return;
        }
