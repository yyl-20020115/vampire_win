...
void Property::add(UnitList* units)
{
  ...
  if (formulas() > 0) { // FOF, either FEQ or FNE
    if (_equalityAtoms == 0) {
      _category = Shell::Property::Category::FNE;
    }
    else {
      _category = Shell::Property::Category::FEQ;
    }
  }
  // no formulas in the input, one of NEQ, HEQ, PEQ, HNE, NNE, EPR, UEQ
	...
	_category = Shell::Property::Category::UEQ;
      }
      else {
	_category = Shell::Property::Category::PEQ;
      }
    }
    else {
      _category = Shell::Property::Category::EPR;
    }
  }
  // one of NEQ, HEQ, PEQ, HNE, NNE, UEQ
  else if (_equationalClauses == 0 ) { // HNE, NNE
    if (clauses() == hornClauses()) { // all clauses are horn
      _category = Shell::Property::Category::HNE;
    }
    else {
      _category = Shell::Property::Category::NNE;
    }
  }
  ...
  // one of NEQ, HEQ
    ...
    _category = Shell::Property::Category::HEQ;
  }
  else {
    _category = Shell::Property::Category::NEQ;
  }
...
void Property::scanSort(unsigned sort)
{
  ...
  if (sort==(unsigned)Sorts::DefaultSorts::SRT_DEFAULT) {
    return;
  }
  _hasNonDefaultSorts = true;
  env.statistics->hasTypes=true;

  if(sort >= (unsigned)Sorts::DefaultSorts::FIRST_USER_SORT){
  ...
  case (unsigned)Sorts::DefaultSorts::SRT_INTEGER:
    addProp(PR_HAS_INTEGERS);
    break;
  case (unsigned)Sorts::DefaultSorts::SRT_RATIONAL:
    addProp(PR_HAS_RATS);
    break;
  case (unsigned)Sorts::DefaultSorts::SRT_REAL:
    addProp(PR_HAS_REALS);
    break;
  case (unsigned)Sorts::DefaultSorts::SRT_BOOL:
    addProp(PR_HAS_BOOLEAN_VARIABLES);
    _hasFOOL = true;
    break;
  }
...
void Property::scanForInterpreted(Term* t)
{
  ...
  if((int)itp < _interpretationPresence.size()){
    _interpretationPresence[(int)itp] = true;
  }
  ...
  if(Theory::isInequality(itp)){
    switch(sort){
      case (unsigned)Sorts::DefaultSorts::SRT_INTEGER : addProp(PR_INTEGER_COMPARISON);
        break;
      case (unsigned)Sorts::DefaultSorts::SRT_RATIONAL : addProp(PR_RAT_COMPARISON);
        break;
      case (unsigned)Sorts::DefaultSorts::SRT_REAL : addProp(PR_REAL_COMPARISON);
        break;
    }
  }
  else if(Theory::isLinearOperation(itp)){
    switch(sort){
      case (unsigned)Sorts::DefaultSorts::SRT_INTEGER : addProp(PR_INTEGER_LINEAR);
        break;
      case (unsigned)Sorts::DefaultSorts::SRT_RATIONAL : addProp(PR_RAT_LINEAR);
        break;
      case (unsigned)Sorts::DefaultSorts::SRT_REAL : addProp(PR_REAL_LINEAR);
        break;
    }
  }
  else if(Theory::isNonLinearOperation(itp)){
    switch(sort){
      case (unsigned)Sorts::DefaultSorts::SRT_INTEGER : addProp(PR_INTEGER_NONLINEAR);
        break;
      case (unsigned)Sorts::DefaultSorts::SRT_RATIONAL : addProp(PR_RAT_NONLINEAR);
        break;
      case (unsigned)Sorts::DefaultSorts::SRT_REAL : addProp(PR_REAL_NONLINEAR);
        break;
    }
  }
}
