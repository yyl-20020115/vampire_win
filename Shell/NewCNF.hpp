...
private:
  ...
  inline void pushLiteral(SPGenClause gc, GenLit gl) {
    ...
    if (formula(gl)->connective() == Connective::LITERAL) {
      /**
    ...
    } else if (formula(gl)->connective() == Connective::NOT) {
      gl = GenLit(formula(gl)->uarg(), OPPOSITE(sign(gl)));
    }

    Formula* f = formula(gl);

    if (f->connective() == Connective::LITERAL && f->literal()->shared()) {
      Literal* l = f->literal();
  ...
  Occurrence pop(Occurrences &occurrences) {
    ...
    while (glit.hasNext()) {
      ...
      if (f->connective() == Connective::LITERAL && f->literal()->shared()) continue;
  ...
  void enqueue(Formula* formula, Occurrences occurrences = Occurrences()) {
    if ((formula->connective() == Connective::LITERAL) && formula->literal()->shared()) return;
    ...
    if (formula->connective() == Connective::NOT) {
      ...
      ASS_REP(formula->connective() != Connective::LITERAL, formula->toString());
