...
Problem* UIHelper::getInputProblem(const Options& opts)
{
  ...
  TimeCounter tc1(Lib::TimeCounterUnit::TC_PARSING);
  env.statistics->phase = Statistics::ExecutionPhase::PARSING;

  SMTLIBLogic smtLibLogic = SMTLIBLogic::SMT_UNDEFINED;
  ...
  case Statistics::TerminationReason::REFUTATION:
    ...
    if (env.options->showInterpolant()!=Options::InterpolantMode::OFF) {
      ...
      case Options::InterpolantMode::OLD_OPT:
        ...
        interpolant = InterpolantMinimizer(InterpolantMinimizer::OptimizationTarget::OT_WEIGHT, false, true, "Minimized interpolant weight").getInterpolant(formulifiedRefutation);
...
void UIHelper::outputSortDeclarations(ostream& out)
{
  ...
  for (unsigned sort = (unsigned)Sorts::DefaultSorts:: SRT_BOOL; sort < sorts; ++sort) {
    if (sort < (unsigned)Sorts::DefaultSorts::FIRST_USER_SORT && ((sort != (unsigned)Sorts::DefaultSorts::SRT_BOOL) || !env.options->showFOOL())) {
      continue;
    }
