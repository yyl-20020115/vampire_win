...
bool ConjunctionGoalAnswerExractor::tryGetAnswer(Clause* refutation, Stack<TermList>& answer)
{
  ...
  if(form->connective()!= Connective::NOT) {
    return false;
  }
  form = form->uarg();
  if(form->connective()!= Connective::EXISTS) {
    return false;
  }
  ...
  if(form->connective()== Connective::LITERAL) {
    goalLits.push(form->literal());
  }
  else if(form->connective()== Connective::AND) {
    FormulaList::Iterator git(form->args());
    while(git.hasNext()) {
      Formula* gf = git.next();
      if(gf->connective()!= Connective::LITERAL) {
        return false;
      }
...
Unit* AnswerLiteralManager::tryAddingAnswerLiteral(Unit* unit)
{
  ...
  if(form->connective()!= Connective::NOT || form->uarg()->connective()!= Connective::EXISTS) {
    return unit;
  }
  ...
  Formula* conj = new JunctionFormula(Connective::AND, conjArgs);
  Formula* newQuant = new QuantifiedFormula(Connective::EXISTS, vars, 0,conj);
