...
bool FunctionDefinition::removeUnusedDefinitions(UnitList*& units, Problem* prb)
{
  ...
  while(defStack.isNonEmpty()) {
	...
	prb->addEliminatedFunction(d->fun, (*d->defCl)[0]);
      ...
      ASS_EQ(d->mark, Def::Mark::UNTOUCHED);
...
//	cout<<"Found: "<<(*(*d->defCl)[0])<<endl;
  ...
  while(dit.hasNext()) {
    ...
    if(d->mark== Def::Mark::SAFE || d->mark== Def::Mark::BLOCKED) {
      continue;
    }
    ASS(d->mark== Def::Mark::UNTOUCHED);
    checkDefinitions(d);
  }

  while(_blockedDefs.isNonEmpty()) {
    Def* d=_blockedDefs.pop();
    ASS_EQ(d->mark, Def::Mark::BLOCKED);
...
void FunctionDefinition::checkDefinitions(Def* def0)
{
  ...
  for(;;) {
    ...
    if(t.isEmpty()) {
      ...
      defArgStack.pop();
      termArgStack.pop();
      ASS(!d || d->mark== Def::Mark::LOOP);
      if(d && d->mark== Def::Mark::LOOP) {
	//the definition is safe (i.e. doesn't contain cycle of non-blocked definitions)
	assignArgOccursData(d);
	_safeDefs.push(d);
	d->mark= Def::Mark::SAFE;
      }
    } else if(t.isTerm()) {
      Term* trm=t.term();
      Def* checkedDef=0;
    toplevel_def:
      if(!_defs.find(trm->functor(), d) || d->mark== Def::Mark::BLOCKED) {
	d=0;
      }
	...
	termArgStack.push(trm);
      ...
      if(d) {
	if(d->mark== Def::Mark::UNTOUCHED) {
	  //enter the definition
	  d->mark= Def::Mark::LOOP;
	  trm=d->rhs;
	  checkedDef=d;
	  goto toplevel_def;
	} else if(d->mark== Def::Mark::LOOP) {
    ...
    if(ts->isNonEmpty()) {
      ...
      if(argDef) {
	ASS_EQ(argDef->mark,Def::Mark::SAFE);
...
Term* FunctionDefinition::applyDefinitions(Literal* lit, Stack<Def*>* usedDefs)
{
  ...
  for(;;) {
    ...
    Def* d;
    if( !defIndex && _defs.find(t->functor(), d) && d->mark!= Def::Mark::BLOCKED) {
      ASS_EQ(d->mark, Def::Mark::UNFOLDED);
      usedDefs->push(d);
      if (env.options->showPreprocessing()) {
        env.beginOutput();
...
FunctionDefinition::isFunctionDefinition (FormulaUnit& unit)
{
  ...
  while (f->connective() == Kernel::Connective::FORALL) {
    f = f->qarg();
  }
