...
void PredicateDefinition::removeUnusedDefinitionsAndPurePredicates(UnitList*& units)
{
  ...
  while(replaceIterator.hasNext()) {
    ...
    if(!v || ( !v->isClause() && static_cast<FormulaUnit*>(v)->formula()->connective()== Connective::_TRUE ) ) {
      replaceIterator.del();
    }
    else {
...
Formula* PredicateDefinition::replacePurePredicates(Formula* f)
{
  ...
  case Connective::OR:
  {
    ...
    while(mergees.isNonEmpty()) {
      ...
      while (it.hasNext()) {
        ...
        ASS_NEQ(g->connective(), Connective::_TRUE);
        ASS_NEQ(g->connective(), Connective::_FALSE);
  	gs.push(g);
      }
    }
    ...
    switch (gs.size()) {
    case 0:
      return new Formula(con == Connective::OR ? false : true);
    case 1:
      return gs.top();
    default:
      FormulaList* res = 0;
  ...
  case Connective::IMP:
  {
    ...
    if (right->connective() == Connective::_TRUE) {
      return right;
    }
    Formula* left = replacePurePredicates(f->left());

    switch (left->connective()) {
    case Connective::_TRUE: // T -> R
      return right;
    case Connective::_FALSE: // F -> R
      return new Formula(true);
    default: // L -> R;
      break;
    }

    if (right->connective() == Connective::_FALSE) {
  ...
  case Connective::XOR:
    {
      ...
      switch (lc) {
      case Connective::_FALSE: // F * _
	switch (rc) {
	case Connective::_FALSE: // F * F
	  return con == Connective::XOR
	         ? right
	         : new Formula(true);
	case Connective::_TRUE: // F * T
	  return con == Connective::XOR
	         ? right
     	         : left;
	default: // F * R
      ...
      case Connective::_TRUE: // T * _
	switch (rc) {
	case Connective::_FALSE: // T * F
	  return con == Connective::XOR
	         ? left
	         : right;
	case Connective::_TRUE: // T * T
	  return con == Connective::XOR
 	         ? new Formula(false)
     	         : left;
	default: // T * R
      ...
      default: // L * _
	switch (rc) {
	case Connective::_FALSE: // L * F
	  return con == Connective::XOR
	         ? left
 	         : new NegatedFormula(left);
	case Connective::_TRUE:  // L * T
	  return con == Connective::XOR
 	         ? new NegatedFormula(left)
     	         : left;
	default:    // L * R
...
FormulaUnit* PredicateDefinition::makeImplFromDef(FormulaUnit* def, unsigned pred, bool forward)
{
  ...
  bool isQuantified=f0->connective()== Connective::FORALL;
  if(isQuantified) {
    f=f0->qarg();
  } else {
    f=f0;
  ...
  switch (f->connective()) {
    case Connective::IFF: {
      if(f->left()->connective()== Connective::LITERAL && f->left()->literal()->functor()==pred) {
        lhs=f->left();
        rhs=f->right();
      } else {
        lhs = f->right();
  ...
  if(lhs->connective()!= Connective::LITERAL || lhs->literal()->functor()!=pred) {
    return 0;
  }
  ...
  Formula* resf;
  if(swapArgs) {
    resf=new BinaryFormula(Connective::IMP, rhs, lhs);
  } else {
    resf=new BinaryFormula(Connective::IMP, lhs, rhs);
  }

  Formula* resf0;
  if(isQuantified) {
    resf0=new QuantifiedFormula(Connective::FORALL, f0->vars(), f0->sorts(), resf);
  } else {
    resf0=resf;
  }
...
void PredicateDefinition::scan(FormulaUnit* unit)
{
  ...
  switch (f->connective()) {
    case Connective::IFF:
      if(f->left()->connective()== Connective::LITERAL) {
        if(tryGetDef(f->left()->literal(), f->right(), unit)) {
          return;
        }
      }
      if(f->right()->connective()== Connective::LITERAL) {
