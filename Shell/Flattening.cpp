  ...
  case Connective::LITERAL:
    {
      ...
      if (env.options->newCNF()) {
        ...
        if (lit->isEquality()) {
          ...
          bool varEquality = lit->isTwoVarEquality() && lit->twoVarEqSort() == (unsigned)Sorts::DefaultSorts::SRT_BOOL;
          ...
          if (lhsBoolean || rhsBoolean || varEquality) {
            ...
            return flatten(new BinaryFormula(lit->polarity() ? Connective::IFF : Connective::XOR, lhsFormula, rhsFormula));
          }
        }
      }
  ...
  case Connective::NOT:
    {
      ...
      if(arg->connective()== Connective::NOT) {
	//two negations cancel each other out
	return arg->uarg();
      }
      if(arg->connective()== Connective::LITERAL) {
				  ...
				  Connective con)
{
  ...
  ASS(con == Connective::OR || con == Connective::AND);
