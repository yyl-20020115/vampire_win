...
vstring TPTPPrinter::getBodyStr(Unit* u)
{
  ...
  if(u->isClause()) {
	...
	res << 'X' << var;
	if(varSort!=(unsigned)Sorts::DefaultSorts::SRT_DEFAULT) {
...
void TPTPPrinter::ensureNecesarySorts()
{
  ...
  for (i = (unsigned)Sorts::DefaultSorts::FIRST_USER_SORT; i < sorts; i++) {
...
vstring TPTPPrinter::toString(const Formula* formula)
{
  ...
  stack.push(make_pair(Kernel::Connective::NOCONN,formula));

  while (stack.isNonEmpty()) {
    Todo todo = stack.pop();

    // in any case start by rendering the connective passed from "above"
    res += names[(unsigned)todo.first];
    ...
    case Connective::OR:
      {
        ...
        res += "(";
        stack.push(make_pair(Kernel::Connective::NOCONN,nullptr)); // render the final closing bracket
        while (FormulaList::isNonEmpty(fs)) {
          ...
          stack.push(make_pair(FormulaList::isNonEmpty(fs) ? c : Kernel::Connective::NOCONN,arg));
        }
    ...
    case Connective::XOR:
      ...
      stack.push(make_pair(Kernel::Connective::NOCONN,f->left())); // first argument without con
    ...
    case Connective::EXISTS:
      {
        vstring result = vstring("(") + names[(unsigned)c] + "[";
        ...
        while (vs.hasNext()) {
          ...
          if (hasSorts) {
            ...
            if (t != (unsigned)Sorts::DefaultSorts::SRT_DEFAULT) {
              ...
              t) && t != (unsigned)Sorts::DefaultSorts::SRT_DEFAULT) {
            result += " : " + env.sorts->sortName(t);
          }
          needsComma = true;
        }
        ...
        stack.push(make_pair(Kernel::Connective::NOCONN,nullptr));
        stack.push(make_pair(Kernel::Connective::NOCONN,nullptr)); // here we close two brackets

        stack.push(make_pair(Kernel::Connective::NOCONN,f->qarg()));
...
//  Inference::Rule rule = inf->rule();
  ...
  if (unit->isClause()) {
    ...
    if(negate_formula) {
      ...
      if(quant->connective()== Connective::NOT) {
	ASS_EQ(quant, f);
	main = toString(quant->uarg());
      }
      else if(quant->connective()== Connective::LITERAL && quant->literal()->isNegative()){
        ASS_EQ(quant,f);
	...
	neg->destroy();
      ...
      if(quant!=f) {
	ASS_EQ(quant->connective(),Connective::FORALL);
