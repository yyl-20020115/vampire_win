...
Grounding::GroundingApplicator::GroundingApplicator()
{
  ...
  for(int i=0;i<funcs;i++) {
    if(env.signature->functionArity(i)==0) {
      if(env.signature->getFunction(i)->fnType()->result()!= (unsigned)Sorts::DefaultSorts::SRT_DEFAULT){
...
*/
  getLocalEqualityAxioms((unsigned)Sorts::DefaultSorts::SRT_DEFAULT, otherThanReflexivity, res);
