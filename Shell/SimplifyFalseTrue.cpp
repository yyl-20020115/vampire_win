          ...
          for (unsigned argument : { 0u, 1u }) {
            if (arguments[argument].isTerm()) {
              ...
              if (isTrue || isFalse) {
                ...
                return (isTrue == (bool)literal->polarity()) 
                    ? simplifiedFormula
  ...
  if (term->isSpecial()) {
    ...
    switch (sd->getType()) {
      ...
      case Term::SF_ITE: {
        ...
        TermList branches[2];
        bool isTrue[2] = { true,true };
        bool isFalse[2] = { true,true };
        ...
        for (BRANCH branch : {THEN, ELSE }) {
          if (isTrue[branch] || isFalse[branch]) {
            ...
            if (isTrue[counterpart] || isFalse[counterpart]) {
              ...
              Formula* junction = new JunctionFormula(isTrue[branch] ? Connective::OR : Connective::AND, args);
