...
Formula* Naming::apply_iter(Formula* top_f) {
  ...
  TimeCounter tc(Lib::TimeCounterUnit::TC_NAMING);
  ...
  {
    Task t;
    t.fncTag = Shell::Naming::ApplyWhat::APPLY_SUB_TOP;
    t.taskApplySub = {top_f, Shell::Naming::Where::ON_TOP};
    todo_stack.push(t);
  }
  ...
  while(todo_stack.isNonEmpty()) {
    ...
    switch (t.fncTag) {
    case Shell::Naming::ApplyWhat::APPLY_SUB_TOP: {
      ...
      case Connective::AND: {
        ...
        int* negCls = 0;
        if (tas.where == Shell::Naming::Where::UNDER_IFF) {
          mem = ALLOC_UNKNOWN(length * sizeof(int), "Naming::apply");
          negCls = array_new<int>(mem, length);
        }

        t.fncTag = Shell::Naming::ApplyWhat::APPLY_SUB_AND;
        tas.taskAndOr = {cls,negCls};

        // fs = apply_list(fs, where, cls, negCls);
        Task t_new;
        t_new.fncTag = Shell::Naming::ApplyWhat::APPLY_LIST_TOP;
      ...
      case Connective::OR: {
        ...
        if (tas.where == Shell::Naming::Where::ON_TOP) {
          tas.where = Shell::Naming::Where::OTHER;
        }

        t.fncTag = Shell::Naming::ApplyWhat::APPLY_SUB_OR;
        tas.taskAndOr = {cls,negCls};
      ...
      case Connective::XOR: {
        ...
        */
        ...
        t.fncTag = Shell::Naming::ApplyWhat::APPLY_SUB_IFFXOR;
        {
          Task t_new;
          t_new.fncTag = Shell::Naming::ApplyWhat::APPLY_SUB_TOP;
          t_new.taskApplySub.where = Shell::Naming::Where::UNDER_IFF;
      ...
      case Connective::EXISTS: {
        tas.taskForallExists.varFlagSet = tas.f->connective() == Connective::FORALL && !_varsInScope;
        if (tas.taskForallExists.varFlagSet) {
          _varsInScope = true;
        }

        // Formula* g = apply_sub(f->qarg(), where, pos, neg);
        t.fncTag = Shell::Naming::ApplyWhat::APPLY_SUB_FORALLEXISTS;
        {
          Task t_new;
          t_new.fncTag = Shell::Naming::ApplyWhat::APPLY_SUB_TOP;
    ...
    case Shell::Naming::ApplyWhat::APPLY_SUB_AND: {
      TaskApplySub& tas = t.taskApplySub;
      ...
      for (;;) {
        ...
        ASS(tas.where != Shell::Naming::Where::UNDER_IFF || maxNeg > 0);
        // splitWhat & 1 => split due to the positive part
        // splitWhat & 2 => split due to the negative part
        int splitWhat = 0;
        if (tas.where != Shell::Naming::Where::UNDER_IFF || product > 1) { // not all formulas are atomic
          if (tas.where != Shell::Naming::Where::ON_TOP && maxPos > 1 && sum >= _threshold) {
            splitWhat |= 1;
          }
          if (tas.where == Shell::Naming::Where::UNDER_IFF && product >= _threshold && maxNeg > 1) {
            splitWhat |= 2;
          }
        }
        if (!splitWhat) { // no more splits
          if (split) {
            ...
            f = new JunctionFormula(Connective::AND, rs);
            DEALLOC_UNKNOWN(gs, "Naming::apply");
          } else if (fs != f->args()) {
            f = new JunctionFormula(Connective::AND, fs);
          }
        ...
        Formula* nm = introduceDefinition(gs[maxIndex], tas.where == Shell::Naming::Where::UNDER_IFF);
        gs[maxIndex] = nm;
        sand.cls[maxIndex] = 1;
        if (tas.where == Shell::Naming::Where::UNDER_IFF) {
    ...
    case Shell::Naming::ApplyWhat::APPLY_SUB_OR: {
      TaskApplySub& tas = t.taskApplySub;
      ...
      for (;;) {
        ...
        // splitWhat & 2 => split due to the negative part
        int splitWhat = 0;
        if (maxPos > 0 && (tas.where != Shell::Naming::Where::UNDER_IFF || product > 1)) { // not all formulas are atomic
          if (product >= _threshold && maxPos > 1) {
            splitWhat |= 1;
          }
          if (tas.where == Shell::Naming::Where::UNDER_IFF && sum >= _threshold && maxNeg > 1) {
            splitWhat |= 2;
          }
        }
        if (!splitWhat) { // no more splits
          if (split) {
            ...
            f = new JunctionFormula(Connective::OR, rs);
            DEALLOC_UNKNOWN(gs, "Naming::apply");
          } else if (fs != f->args()) {
            f = new JunctionFormula(Connective::OR, fs);
          }
    ...
    case Shell::Naming::ApplyWhat::APPLY_SUB_IFFXOR: {
      TaskApplySub& tas = t.taskApplySub;
      ...
      */
      ...
      bool left; // name left
      if (pos < _threshold) {
        if (tas.where != Shell::Naming::Where::UNDER_IFF || neg < _threshold) {
          if (l != f->left() || r != f->right()) {
            f = new BinaryFormula(con, l, r);
          }
          // return f
    ...
    case Shell::Naming::ApplyWhat::APPLY_SUB_FORALLEXISTS: {
      TaskApplySub& tas = t.taskApplySub;
    ...
    case Shell::Naming::ApplyWhat::APPLY_LIST_TOP: {
    ...
    case Shell::Naming::ApplyWhat::APPLY_LIST_POST: {
      TaskApplyList& tal = t.taskApplyList;
      ...
      Formula* g;
      {
        ...
        if (tal.where == Shell::Naming::Where::UNDER_IFF) {
...
Formula* Naming::apply_sub(Formula* f, Where where, int& pos, int& neg) {
  ...
  case Connective::AND: {
    ...
    int* cls = array_new<int>(mem, length);
    int* negCls = 0;
    if (where == Shell::Naming::Where::UNDER_IFF) {
      mem = ALLOC_UNKNOWN(length * sizeof(int), "Naming::apply");
      negCls = array_new<int>(mem, length);
    }
    fs = apply_list(fs, where, cls, negCls);
    ...
    for (;;) {
      ...
      ASS(currArg == 0);
      ASS(maxPos > 0 || _preserveEpr);
      ASS(where != Shell::Naming::Where::UNDER_IFF || maxNeg > 0);
      // splitWhat & 1 => split due to the positive part
      // splitWhat & 2 => split due to the negative part
      int splitWhat = 0;
      if (where != Shell::Naming::Where::UNDER_IFF || product > 1) { // not all formulas are atomic
        if (where != Shell::Naming::Where::ON_TOP && maxPos > 1 && sum >= _threshold) {
          splitWhat |= 1;
        }
        if (where == Shell::Naming::Where::UNDER_IFF && product >= _threshold && maxNeg > 1) {
          splitWhat |= 2;
        }
      }
      if (!splitWhat) { // no more splits
      ...
      int maxIndex =
          splitWhat == 1 || (splitWhat == 3 && sum > product) || maxNeg == 1 ?
              maxPosIndex : maxNegIndex;
      Formula* nm = introduceDefinition(gs[maxIndex], where == Shell::Naming::Where::UNDER_IFF);
      gs[maxIndex] = nm;
      cls[maxIndex] = 1;
      if (where == Shell::Naming::Where::UNDER_IFF) {
        negCls[maxIndex] = 1;
  ...
  case Connective::OR: {
    ...
    if (where == Shell::Naming::Where::ON_TOP) {
      where = Shell::Naming::Where::OTHER;
    }
    fs = apply_list(fs, where, cls, negCls);
    bool split = false;
    // formula array, initialised only upon demand
    ...
    for (;;) {
      ...
      // splitWhat & 2 => split due to the negative part
      int splitWhat = 0;
      if (maxPos > 0 && (where != Shell::Naming::Where::UNDER_IFF || product > 1)) { // not all formulas are atomic
        if (product >= _threshold && maxPos > 1) {
          splitWhat |= 1;
        }
        if (where == Shell::Naming::Where::UNDER_IFF && sum >= _threshold && maxNeg > 1) {
          splitWhat |= 2;
        }
      }
      if (!splitWhat) { // no more splits
  ...
  case Connective::XOR: {
    ...
    Formula* r;
    if (con == Connective::IFF) {
      l = apply_sub(f->left(), Shell::Naming::Where::UNDER_IFF, posl, negl);
      r = apply_sub(f->right(), Shell::Naming::Where::UNDER_IFF, posr, negr);
    } else {
      l = apply_sub(f->left(), Shell::Naming::Where::UNDER_IFF, negl, posl);
      r = apply_sub(f->right(), Shell::Naming::Where::UNDER_IFF, negr, posr);
    }
    if (where == Shell::Naming::Where::ON_TOP
        && ((posl == 1 && posr == 1) || (negl == 1 && negr == 1))) {
      if (l != f->left() || r != f->right()) {
    ...
    bool left; // name left
    if (pos < _threshold) {
      if (where != Shell::Naming::Where::UNDER_IFF || neg < _threshold) {
        if (l != f->left() || r != f->right()) {
          f = new BinaryFormula(con, l, r);
        }
  ...
  case Connective::FORALL:
  case Connective::EXISTS: {
    bool varFlagSet = f->connective() == Connective::FORALL && !_varsInScope;
    if (varFlagSet) {
      _varsInScope = true;
    }
    Formula* g = apply_sub(f->qarg(), where, pos, neg);
...
bool Naming::canBeInDefinition(Formula* f, Where where) {
  ...
  while (sfit.hasNext()) {
    ...
    if (sf->connective() == Connective::FORALL) {
      unQuant = true;
    }
    if (sf->connective() == Connective::EXISTS) {
      exQuant = true;
    }
  }
  ...
  if (!_varsInScope && freeVars
      && (exQuant || (unQuant && where == Shell::Naming::Where::UNDER_IFF))) {
    return false;
  }
...
Literal* Naming::getDefinitionLiteral(Formula* f, Formula::VarList* freeVars) {
  ...
  while (vit.hasNext()) {
    ...
    domainSorts.push(varSorts.get(uvar, (unsigned)Sorts::DefaultSorts::SRT_DEFAULT));
...
Formula* Naming::introduceDefinition(Formula* f, bool iff) {
  ...
  ASS_NEQ(f->connective(), Connective::LITERAL);
  ASS_NEQ(f->connective(), Connective::NOT);
  ...
  Formula* def;
  if (iff) {
    def = new BinaryFormula(Connective::IFF, name, f);
  }
  // iff = false
  else {
    FormulaList* fs = f->connective() == Connective::OR ? f->args() : new FormulaList(f);
    Formula* nameFormula = new NegatedFormula(name);
    FormulaList::push(nameFormula, fs);
    def = new JunctionFormula(Connective::OR, fs);
  }
  if (Formula::VarList::isNonEmpty(vs)) {
    //TODO do we know the sorts of the free variabls vs?
    def = new QuantifiedFormula(Connective::FORALL, vs, 0, def);
  }
