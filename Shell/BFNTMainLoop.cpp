...
void BFNTMainLoop::runChild(size_t modelSize)
{
  ...
  if(env.options->mode()!=Options::Mode::SPIDER) {
    if(innerRes.terminationReason==Statistics::TerminationReason::SATISFIABLE || innerRes.terminationReason==Statistics::TerminationReason::TIME_LIMIT) {
      env.beginOutput();
  ...
  if(env.options->mode()!=Options::Mode::SPIDER && innerRes.terminationReason==Statistics::TerminationReason::SATISFIABLE) {
    env.beginOutput();
...
MainLoopResult BFNTMainLoop::spawnChild(size_t modelSize)
{
...
#ifdef _WIN32
  //TODO:
  return MainLoopResult(Statistics::TerminationReason::UNKNOWN);
#else
  int status;
  errno=0;
  ...
  if(WIFEXITED(status)) {
    ...
    case BFNT_CHILD_RESULT_UNSAT:
      return MainLoopResult(Statistics::TerminationReason::REFUTATION);
...
MainLoopResult BFNTMainLoop::runImpl()
{
  ...
  for(;;) {
    ...
    if(childResult.terminationReason == Statistics::TerminationReason::SATISFIABLE) {
      return childResult;
    }
