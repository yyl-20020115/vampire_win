...
void TheoryAxioms::addArrayExtensionalityAxioms(unsigned arraySort, unsigned skolemFn)
{
  ...
  unsigned sel = env.signature->getInterpretingSymbol(Theory::Interpretation::ARRAY_SELECT,Theory::getArrayOperatorType(arraySort,Theory::Interpretation::ARRAY_SELECT));
...
void TheoryAxioms::addBooleanArrayExtensionalityAxioms(unsigned arraySort, unsigned skolemFn)
{
  ...
  OperatorType* selectType = Theory::getArrayOperatorType(arraySort,Theory::Interpretation::ARRAY_BOOL_SELECT);

  unsigned sel = env.signature->getInterpretingSymbol(Theory::Interpretation::ARRAY_BOOL_SELECT,selectType);
  ...
  Formula* sx_neq_sy = new BinaryFormula(Connective::XOR, sel_x_sk, sel_y_sk); //select(x,sk(x,y)) <~> select(y,sk(x,y))

  Formula* axiom = new QuantifiedFormula(Connective::FORALL, new Formula::VarList(0, new Formula::VarList(1, 0)),
                                         new Formula::SortList(arraySort, new Formula::SortList(arraySort,0)),
                                         new BinaryFormula(Connective::IMP, x_neq_y, sx_neq_sy));
...
void TheoryAxioms::addArrayWriteAxioms(unsigned arraySort)
{
  ...
  unsigned func_select = env.signature->getInterpretingSymbol(Theory::Interpretation::ARRAY_SELECT,Theory::getArrayOperatorType(arraySort,Theory::Interpretation::ARRAY_SELECT));
  unsigned func_store = env.signature->getInterpretingSymbol(Theory::Interpretation::ARRAY_STORE,Theory::getArrayOperatorType(arraySort,Theory::Interpretation::ARRAY_STORE));
...
void TheoryAxioms::addBooleanArrayWriteAxioms(unsigned arraySort)
{
  ...
  unsigned pred_select = env.signature->getInterpretingSymbol(Theory::Interpretation::ARRAY_BOOL_SELECT,Theory::getArrayOperatorType(arraySort,Theory::Interpretation::ARRAY_BOOL_SELECT));
  unsigned func_store = env.signature->getInterpretingSymbol(Theory::Interpretation::ARRAY_STORE,Theory::getArrayOperatorType(arraySort,Theory::Interpretation::ARRAY_STORE));
  ...
  Formula* writeEq = new BinaryFormula(Connective::IFF, sWJ, sAJ);//(select(store(A,I,V), J) <=> select(A,J)
  Formula* ax2 = new BinaryFormula(Connective::IMP, indexEq, writeEq);
...
void TheoryAxioms::apply()
{
  ...
  bool modified = false;
  bool haveIntPlus =
    prop->hasInterpretedOperation(Theory::Interpretation::INT_PLUS) ||
    prop->hasInterpretedOperation(Theory::Interpretation::INT_UNARY_MINUS) ||
    prop->hasInterpretedOperation(Theory::Interpretation::INT_LESS) ||
    prop->hasInterpretedOperation(Theory::Interpretation::INT_MULTIPLY);
  bool haveIntMultiply =
    prop->hasInterpretedOperation(Theory::Interpretation::INT_MULTIPLY);

  bool haveIntDivision =
    prop->hasInterpretedOperation(Theory::Interpretation::INT_QUOTIENT_E) || // let's ignore the weird _F and _T for now!
    prop->hasInterpretedOperation(Theory::Interpretation::INT_REMAINDER_E) ||
    prop->hasInterpretedOperation(Theory::Interpretation::INT_ABS);

  bool haveIntDivides = prop->hasInterpretedOperation(Theory::Interpretation::INT_DIVIDES);

  bool haveIntFloor = prop->hasInterpretedOperation(Theory::Interpretation::INT_FLOOR);
  bool haveIntCeiling = prop->hasInterpretedOperation(Theory::Interpretation::INT_CEILING);
  bool haveIntRound = prop->hasInterpretedOperation(Theory::Interpretation::INT_ROUND);
  bool haveIntTruncate = prop->hasInterpretedOperation(Theory::Interpretation::INT_TRUNCATE);
  ...
  if (haveIntPlus || haveIntUnaryRoundingFunction || haveIntDivision || haveIntDivides) {
    ...
    if(haveIntMultiply || haveIntDivision || haveIntDivides) {
      addAdditionOrderingAndMultiplicationAxioms(Theory::Interpretation::INT_PLUS, Theory::Interpretation::INT_UNARY_MINUS, zero, one,
						 Theory::Interpretation::INT_LESS, Theory::Interpretation::INT_MULTIPLY);
      if(haveIntDivision){
        addIntegerDivisionWithModuloAxioms(Theory::Interpretation::INT_PLUS, Theory::Interpretation::INT_UNARY_MINUS, Theory::Interpretation::INT_LESS,
                                 Theory::Interpretation::INT_MULTIPLY, Theory::Interpretation::INT_QUOTIENT_E, Theory::Interpretation::INT_DIVIDES,
                                 Theory::Interpretation::INT_REMAINDER_E, Theory::Interpretation::INT_ABS, zero,one);
      }
        ...
        while(nsit.hasNext()){
          ...
          addIntegerDividesAxioms(Theory::Interpretation::INT_DIVIDES,Theory::Interpretation::INT_MULTIPLY,zero,n); 
        }
      }
    }
    else {
      addAdditionAndOrderingAxioms(Theory::Interpretation::INT_PLUS, Theory::Interpretation::INT_UNARY_MINUS, zero, one,
				   Theory::Interpretation::INT_LESS);
    }
    addExtraIntegerOrderingAxiom(Theory::Interpretation::INT_PLUS, one, Theory::Interpretation::INT_LESS);
    modified = true;
  }
  bool haveRatPlus =
    prop->hasInterpretedOperation(Theory::Interpretation::RAT_PLUS) ||
    prop->hasInterpretedOperation(Theory::Interpretation::RAT_UNARY_MINUS) ||
    prop->hasInterpretedOperation(Theory::Interpretation::RAT_LESS) ||
    prop->hasInterpretedOperation(Theory::Interpretation::RAT_QUOTIENT) ||
    prop->hasInterpretedOperation(Theory::Interpretation::RAT_MULTIPLY);
  bool haveRatMultiply =
    prop->hasInterpretedOperation(Theory::Interpretation::RAT_MULTIPLY);
  bool haveRatQuotient =
    prop->hasInterpretedOperation(Theory::Interpretation::RAT_QUOTIENT);

  bool haveRatFloor = prop->hasInterpretedOperation(Theory::Interpretation::RAT_FLOOR);
  bool haveRatCeiling = prop->hasInterpretedOperation(Theory::Interpretation::RAT_CEILING);
  bool haveRatRound = prop->hasInterpretedOperation(Theory::Interpretation::RAT_ROUND);
  bool haveRatTruncate = prop->hasInterpretedOperation(Theory::Interpretation::RAT_TRUNCATE);
  ...
  if (haveRatPlus || haveRatUnaryRoundingFunction) {
    ...
    if(haveRatMultiply || haveRatRound || haveRatQuotient) {
      addAdditionOrderingAndMultiplicationAxioms(Theory::Interpretation::RAT_PLUS, Theory::Interpretation::RAT_UNARY_MINUS, zero, one,
						 Theory::Interpretation::RAT_LESS, Theory::Interpretation::RAT_MULTIPLY);

      if(haveRatQuotient){
        addQuotientAxioms(Theory::Interpretation::RAT_QUOTIENT,Theory::Interpretation::RAT_MULTIPLY,zero,one,Theory::Interpretation::RAT_LESS);
      }
    }
    else {
      addAdditionAndOrderingAxioms(Theory::Interpretation::RAT_PLUS, Theory::Interpretation::RAT_UNARY_MINUS, zero, one,
				   Theory::Interpretation::RAT_LESS);
    }
    if(haveRatFloor || haveRatRound){
      addFloorAxioms(Theory::Interpretation::RAT_FLOOR,Theory::Interpretation::RAT_LESS,Theory::Interpretation::RAT_UNARY_MINUS,Theory::Interpretation::RAT_PLUS,one);
    }
    if(haveRatCeiling || haveRatRound){
      addCeilingAxioms(Theory::Interpretation::RAT_CEILING,Theory::Interpretation::RAT_LESS,Theory::Interpretation::RAT_PLUS,one);
    }
    if(haveRatRound){
      //addRoundAxioms(Theory::Interpretation::INT_TRUNCATE,Theory::Interpretation::INT_FLOOR,Theory::Interpretation::INT_CEILING);
    }
    if(haveRatTruncate){
      addTruncateAxioms(Theory::Interpretation::RAT_TRUNCATE,Theory::Interpretation::RAT_LESS,Theory::Interpretation::RAT_UNARY_MINUS,
                        Theory::Interpretation::RAT_PLUS,zero,one);
    }
    modified = true;
  }
  bool haveRealPlus =
    prop->hasInterpretedOperation(Theory::Interpretation::REAL_PLUS) ||
    prop->hasInterpretedOperation(Theory::Interpretation::REAL_UNARY_MINUS) ||
    prop->hasInterpretedOperation(Theory::Interpretation::REAL_LESS) ||
    prop->hasInterpretedOperation(Theory::Interpretation::REAL_QUOTIENT) ||
    prop->hasInterpretedOperation(Theory::Interpretation::REAL_MULTIPLY);
  bool haveRealMultiply =
    prop->hasInterpretedOperation(Theory::Interpretation::REAL_MULTIPLY);
  bool haveRealQuotient =
    prop->hasInterpretedOperation(Theory::Interpretation::REAL_QUOTIENT);

  bool haveRealFloor = prop->hasInterpretedOperation(Theory::Interpretation::REAL_FLOOR);
  bool haveRealCeiling = prop->hasInterpretedOperation(Theory::Interpretation::REAL_CEILING);
  bool haveRealRound = prop->hasInterpretedOperation(Theory::Interpretation::REAL_ROUND);
  bool haveRealTruncate = prop->hasInterpretedOperation(Theory::Interpretation::REAL_TRUNCATE);
  ...
  if (haveRealPlus || haveRealUnaryRoundingFunction) {
    ...
    if(haveRealMultiply || haveRealQuotient) {
      addAdditionOrderingAndMultiplicationAxioms(Theory::Interpretation::REAL_PLUS, Theory::Interpretation::REAL_UNARY_MINUS, zero, one,
						 Theory::Interpretation::REAL_LESS, Theory::Interpretation::REAL_MULTIPLY);

      if(haveRealQuotient){
        addQuotientAxioms(Theory::Interpretation::REAL_QUOTIENT,Theory::Interpretation::REAL_MULTIPLY,zero,one,Theory::Interpretation::REAL_LESS);
      }
    }
    else {
      addAdditionAndOrderingAxioms(Theory::Interpretation::REAL_PLUS, Theory::Interpretation::REAL_UNARY_MINUS, zero, one,
				   Theory::Interpretation::REAL_LESS);
    }
    if(haveRealFloor || haveRealRound){
      addFloorAxioms(Theory::Interpretation::REAL_FLOOR,Theory::Interpretation::REAL_LESS,Theory::Interpretation::REAL_UNARY_MINUS,Theory::Interpretation::REAL_PLUS,one);
    }
    if(haveRealCeiling || haveRealRound){
      addCeilingAxioms(Theory::Interpretation::REAL_CEILING,Theory::Interpretation::REAL_LESS,Theory::Interpretation::REAL_PLUS,one);
    }
    if(haveRealRound){
      //addRoundAxioms(Theory::Interpretation::INT_TRUNCATE,Theory::Interpretation::INT_FLOOR,Theory::Interpretation::INT_CEILING);
    }
    if(haveRealTruncate){
      addTruncateAxioms(Theory::Interpretation::REAL_TRUNCATE,Theory::Interpretation::REAL_LESS,Theory::Interpretation::REAL_UNARY_MINUS,
                        Theory::Interpretation::REAL_PLUS,zero,one);
    }
  ...
  while(arraySorts.hasNext()){
    ...
    bool isBool = (env.sorts->getArraySort(arraySort)->getInnerSort() ==(unsigned) Sorts::DefaultSorts::SRT_BOOL);
    
    // Check if they are used
    Interpretation arraySelect = isBool ? Theory::Interpretation::ARRAY_BOOL_SELECT : Theory::Interpretation::ARRAY_SELECT;
    bool haveSelect = prop->hasInterpretedOperation(arraySelect,Theory::getArrayOperatorType(arraySort,arraySelect));
    bool haveStore = prop->hasInterpretedOperation(Theory::Interpretation::ARRAY_STORE,Theory::getArrayOperatorType(arraySort,Theory::Interpretation::ARRAY_STORE));
...
void TheoryAxioms::applyFOOL() {
  ...
  // Add "$$true != $$false"
  Literal* tneqf = Literal::createEquality(false, t, f, (unsigned)Sorts::DefaultSorts::SRT_BOOL);
  ...
  Literal* boolVar1 = Literal::createEquality(true, TermList(0, false), t, (unsigned)Sorts::DefaultSorts::SRT_BOOL);
  Literal* boolVar2 = Literal::createEquality(true, TermList(0, false), f, (unsigned)Sorts::DefaultSorts::SRT_BOOL);
...
void TheoryAxioms::addExhaustivenessAxiom(TermAlgebra* ta) {
  ...
  for (unsigned i = 0; i < ta->nConstructors(); i++) {
    ...
    for (unsigned j = 0; j < c->arity(); j++) {
      if (c->argSort(j) == (unsigned)Sorts::DefaultSorts::SRT_BOOL) {
        addsFOOL = true;
