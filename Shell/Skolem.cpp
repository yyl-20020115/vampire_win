  ...
  case Connective::EXISTS:
    {
      ...
      */
      ...
      while (vs.hasNext()) {
        int v = vs.next();
        unsigned rangeSort=_varSorts.get(v, (unsigned)Sorts::DefaultSorts::SRT_DEFAULT);
...

      {
        ...
        Formula* def = new BinaryFormula(Connective::IMP, before, after);

        if (arity > 0) {
          def = new QuantifiedFormula(Connective::FORALL,var_args,nullptr,def);
        }
