...
private:
  ...
  struct ParseResult {
    ...
    ParseResult(Formula* frm) : sort((unsigned)Sorts::DefaultSorts::SRT_BOOL), formula(true), frm(frm) {}
     ...
     * and return its vampire sort (which may be (unsigned)Sorts::DefaultSorts::SRT_BOOL).
     */
