...
: _logicSet(false),
  _logic(Shell::SMTLIBLogic::SMT_UNDEFINED),
...
SMTLIBLogic SMTLIB2::getLogicFromString(const vstring& str)
{
  ...
  ASS_EQ(smtlibLogicNames.length, (int)SMTLIBLogic::SMT_UNDEFINED);

  int res = smtlibLogicNames.tryToFind(str.c_str());
  if(res==-1) {
    return SMTLIBLogic::SMT_UNDEFINED;
  }
...
void SMTLIB2::readLogic(const vstring& logicStr)
{
  ...
  switch (_logic) {
  case Shell::SMTLIBLogic::SMT_ALL:
  case Shell::SMTLIBLogic::SMT_ALIA:
  case Shell::SMTLIBLogic::SMT_AUFDTLIA:
  case Shell::SMTLIBLogic::SMT_AUFDTLIRA:
  case Shell::SMTLIBLogic::SMT_AUFDTNIRA:
  case Shell::SMTLIBLogic::SMT_AUFLIA:
  case Shell::SMTLIBLogic::SMT_AUFNIA:
  case Shell::SMTLIBLogic::SMT_AUFLIRA:
  case Shell::SMTLIBLogic::SMT_AUFNIRA:
  case Shell::SMTLIBLogic::SMT_LIA:
  case Shell::SMTLIBLogic::SMT_NIA:
  case Shell::SMTLIBLogic::SMT_QF_ALIA:
  case Shell::SMTLIBLogic::SMT_QF_ANIA:
  case Shell::SMTLIBLogic::SMT_QF_AUFLIA:
  case Shell::SMTLIBLogic::SMT_QF_AUFNIA:
  case Shell::SMTLIBLogic::SMT_QF_AX:
  case Shell::SMTLIBLogic::SMT_QF_IDL:
  case Shell::SMTLIBLogic::SMT_QF_LIA:
  case Shell::SMTLIBLogic::SMT_QF_LIRA:
  case Shell::SMTLIBLogic::SMT_QF_NIA:
  case Shell::SMTLIBLogic::SMT_QF_NIRA:
  case Shell::SMTLIBLogic::SMT_QF_UF:
  case Shell::SMTLIBLogic::SMT_QF_UFIDL:
  case Shell::SMTLIBLogic::SMT_QF_UFLIA:
  case Shell::SMTLIBLogic::SMT_QF_UFNIA:
  case Shell::SMTLIBLogic::SMT_UF:
  case Shell::SMTLIBLogic::SMT_UFDT:
  case Shell::SMTLIBLogic::SMT_UFDTLIA:
  case Shell::SMTLIBLogic::SMT_UFDTLIRA:
  case Shell::SMTLIBLogic::SMT_UFDTNIA:
  case Shell::SMTLIBLogic::SMT_UFDTNIRA:
  case Shell::SMTLIBLogic::SMT_UFIDL:
  case Shell::SMTLIBLogic::SMT_UFLIA:
  case Shell::SMTLIBLogic::SMT_UFNIA:
    break;

  // pure real arithmetic theories treat decimals as Real constants
  case Shell::SMTLIBLogic::SMT_LRA:
  case Shell::SMTLIBLogic::SMT_NRA:
  case Shell::SMTLIBLogic::SMT_QF_LRA:
  case Shell::SMTLIBLogic::SMT_QF_NRA:
  case Shell::SMTLIBLogic::SMT_QF_RDL:
  case Shell::SMTLIBLogic::SMT_QF_UFLRA:
  case Shell::SMTLIBLogic::SMT_QF_UFNRA:
  case Shell::SMTLIBLogic::SMT_UFLRA:
    _numeralsAreReal = true;
    break;

  // we don't support bit vectors
  case Shell::SMTLIBLogic::SMT_BV:
  case Shell::SMTLIBLogic::SMT_QF_ABV:
  case Shell::SMTLIBLogic::SMT_QF_AUFBV:
  case Shell::SMTLIBLogic::SMT_QF_BV:
  case Shell::SMTLIBLogic::SMT_QF_UFBV:
  case Shell::SMTLIBLogic::SMT_UFBV:
...
SMTLIB2::BuiltInSorts SMTLIB2::getBuiltInSortFromString(const vstring& str)
{
  ...
  ASS_EQ(builtInSortNames.length, (int)BuiltInSorts::BS_INVALID);

  int res = builtInSortNames.tryToFind(str.c_str());
  if(res==-1) {
    return BuiltInSorts::BS_INVALID;
  }
...
bool SMTLIB2::isAlreadyKnownSortSymbol(const vstring& name)
{
  ...
  if (getBuiltInSortFromString(name) != BuiltInSorts::BS_INVALID) {
    return true;
  }
...
unsigned SMTLIB2::declareSort(LExpr* sExpr)
{
  ...
  ASS_EQ((unsigned)Sorts::DefaultSorts::SRT_DEFAULT,0); // there is no default sort in smtlib, so we can use 0 as a results separator
  ...
  todo.push(make_pair(SortParseOperation::SPO_PARSE,sExpr));

  while (todo.isNonEmpty()) {
    pair<SortParseOperation,LExpr*> cur = todo.pop();
    SortParseOperation op = cur.first;

    if (op == SortParseOperation::SPO_POP_LOOKUP) {
      delete lookups.pop();
      forbidden.pop();
      continue;
    }

    if (op == SortParseOperation::SPO_CHECK_ARITY) {
      if (results.size() < 2) {
        goto malformed;
      }
    ...
    ASS_EQ(op, SortParseOperation::SPO_PARSE);
    LExpr* exp = cur.second;

    if (exp->isList()) {
      LExprList::Iterator lIt(exp->list);

      todo.push(make_pair(SortParseOperation::SPO_CHECK_ARITY,nullptr));
      results.push(SEPARATOR);

      while (lIt.hasNext()) {
        todo.push(make_pair(SortParseOperation::SPO_PARSE,lIt.next()));
      }
    } else {
      ASS(exp->isAtom());
      vstring& id = exp->str;
      ...
      if (_sortDefinitions.find(id,def)) {
        ...
        todo.push(make_pair(SortParseOperation::SPO_POP_LOOKUP,nullptr)); //schedule lookup deletion (see above)
        todo.push(make_pair(SortParseOperation::SPO_PARSE,def.body));
      ...
      switch (bs) {
      case BuiltInSorts::BS_BOOL:
          results.push((unsigned)Sorts::DefaultSorts::SRT_BOOL);
          continue;
        case BuiltInSorts::BS_INT:
          results.push((unsigned)Sorts::DefaultSorts::SRT_INTEGER);
          continue;
        case BuiltInSorts::BS_REAL:
          results.push((unsigned)Sorts::DefaultSorts::SRT_REAL);
          continue;
        case BuiltInSorts::BS_ARRAY:
          if (results.size() < 2) {
            goto malformed;
          } else {
        ...
        default:
          ASS_EQ(bs, BuiltInSorts::BS_INVALID);
      }
...
SMTLIB2::FormulaSymbol SMTLIB2::getBuiltInFormulaSymbol(const vstring& str)
{
  ...
  ASS_EQ(formulaSymbolNames.length, (int)FormulaSymbol::FS_USER_PRED_SYMBOL);

  int res = formulaSymbolNames.tryToFind(str.c_str());
  if(res==-1) {
    return FormulaSymbol::FS_USER_PRED_SYMBOL;
  }
...
SMTLIB2::TermSymbol SMTLIB2::getBuiltInTermSymbol(const vstring& str)
{
  ...
  ASS_EQ(termSymbolNames.length, (int)TermSymbol::TS_USER_FUNCTION);

  int resInt = termSymbolNames.tryToFind(str.c_str());
  if(resInt==-1) {
    return TermSymbol::TS_USER_FUNCTION;
  }
...
bool SMTLIB2::isAlreadyKnownFunctionSymbol(const vstring& name)
{
  ...
  if (getBuiltInFormulaSymbol(name) != FormulaSymbol::FS_USER_PRED_SYMBOL) {
    return true;
  }

  if (getBuiltInTermSymbol(name) != TermSymbol::TS_USER_FUNCTION) {
    return true;
  }
...
SMTLIB2::DeclaredFunction SMTLIB2::declareFunctionOrPredicate(const vstring& name, signed rangeSort, const Stack<unsigned>& argSorts)
{
  ...
  if (rangeSort == (unsigned)Sorts::DefaultSorts::SRT_BOOL) { // predicate
...
TermAlgebraConstructor* SMTLIB2::buildTermAlgebraConstructor(vstring constrName, unsigned taSort,
  ...
  for (unsigned i = 0; i < arity; i++) {
    ...
    bool isPredicate = destructorSort == (unsigned)Sorts::DefaultSorts::SRT_BOOL;
    bool added;
...
bool SMTLIB2::ParseResult::asFormula(Formula*& resFrm)
{
  ...
  if (formula) {
    ASS_EQ(sort, (unsigned)Sorts::DefaultSorts::SRT_BOOL);
    resFrm = frm;
  ...
  if (sort == (unsigned)Sorts::DefaultSorts::SRT_BOOL) {
...
Interpretation SMTLIB2::getFormulaSymbolInterpretation(FormulaSymbol fs, unsigned firstArgSort)
{
  ...
  switch(fs) {
  case FormulaSymbol::FS_LESS:
    switch(firstArgSort) {
    case (unsigned)Sorts::DefaultSorts::SRT_INTEGER:
  return Theory::Interpretation::INT_LESS;
    case (unsigned)Sorts::DefaultSorts::SRT_REAL:
  return Theory::Interpretation::REAL_LESS;
    default:
      break;
    }
    break;
  case FormulaSymbol::FS_LESS_EQ:
    switch(firstArgSort) {
    case (unsigned)Sorts::DefaultSorts::SRT_INTEGER:
  return Theory::Interpretation::INT_LESS_EQUAL;
    case (unsigned)Sorts::DefaultSorts::SRT_REAL:
      return Theory::Interpretation::REAL_LESS_EQUAL;
    default:
      break;
    }
    break;
  case FormulaSymbol::FS_GREATER:
    switch(firstArgSort) {
    case (unsigned)Sorts::DefaultSorts::SRT_INTEGER:
      return Theory::Interpretation::INT_GREATER;
    case (unsigned)Sorts::DefaultSorts::SRT_REAL:
      return Theory::Interpretation::REAL_GREATER;
    default:
      break;
    }
    break;
  case FormulaSymbol::FS_GREATER_EQ:
    switch(firstArgSort) {
    case (unsigned)Sorts::DefaultSorts::SRT_INTEGER:
      return Theory::Interpretation::INT_GREATER_EQUAL;
    case (unsigned)Sorts::DefaultSorts::SRT_REAL:
      return Theory::Interpretation::REAL_GREATER_EQUAL;
    default:
      break;
    }
    break;
  ...
  USER_ERROR("invalid sort "+env.sorts->sortName(firstArgSort)+" for interpretation "+vstring(s_formulaSymbolNameStrings[(int)fs]));
}
...
Interpretation SMTLIB2::getUnaryMinusInterpretation(unsigned argSort)
{
  ...
  switch(argSort) {
    case (unsigned)Sorts::DefaultSorts::SRT_INTEGER:
      return Theory::Interpretation::INT_UNARY_MINUS;
    case (unsigned)Sorts::DefaultSorts::SRT_REAL:
      return Theory::Interpretation::REAL_UNARY_MINUS;
    default:
...
Interpretation SMTLIB2::getTermSymbolInterpretation(TermSymbol ts, unsigned firstArgSort)
{
  ...
  switch(ts) {
  case TermSymbol::TS_MINUS:
    switch(firstArgSort) {
    case (unsigned)Sorts::DefaultSorts::SRT_INTEGER:
  return Theory::Interpretation::INT_MINUS;
    case (unsigned)Sorts::DefaultSorts::SRT_REAL:
  return Theory::Interpretation::REAL_MINUS;
    default:
      break;
    }
    break;
  case TermSymbol::TS_PLUS:
    switch(firstArgSort) {
    case (unsigned)Sorts::DefaultSorts::SRT_INTEGER:
  return Theory::Interpretation::INT_PLUS;
    case (unsigned)Sorts::DefaultSorts::SRT_REAL:
      return Theory::Interpretation::REAL_PLUS;
    default:
      break;
    }
    break;
  case TermSymbol::TS_MULTIPLY:
    switch(firstArgSort) {
    case (unsigned)Sorts::DefaultSorts::SRT_INTEGER:
      return Theory::Interpretation::INT_MULTIPLY;
    case (unsigned)Sorts::DefaultSorts::SRT_REAL:
      return Theory::Interpretation::REAL_MULTIPLY;
    default:
      break;
    }
    break;

  case TermSymbol::TS_DIVIDE:
    if (firstArgSort ==(unsigned) Sorts::DefaultSorts::SRT_REAL)
      return Theory::Interpretation::REAL_QUOTIENT;
    break;

  case TermSymbol::TS_DIV:
    if (firstArgSort == (unsigned)Sorts::DefaultSorts::SRT_INTEGER)
      return Theory::Interpretation::INT_QUOTIENT_E;
    break;

  default:
    ASSERTION_VIOLATION_REP(ts);
  }
    USER_ERROR("invalid sort "+env.sorts->sortName(firstArgSort)+" for interpretation "+vstring(s_termSymbolNameStrings[(int)ts]));
}
...
void SMTLIB2::parseLetBegin(LExpr* exp)
{
  ...
  _todo.push(make_pair(Parse::SMTLIB2::ParseOperation::PO_LET_END,exp));

  // this will parse the let's body (in the context of the lookup)
  _todo.push(make_pair(Parse::SMTLIB2::ParseOperation::PO_PARSE,body));

  // this will create the lookup when all bindings' expressions are parsed (and their sorts known)
  _todo.push(make_pair(Parse::SMTLIB2::ParseOperation::PO_LET_PREPARE_LOOKUP,exp));
  ...
  while (bindRdr.hasNext()) {
    ...
    _todo.push(make_pair(Parse::SMTLIB2::ParseOperation::PO_PARSE,expr)); // just parse the expression
    pRdr.acceptEOL();
  }
}
...
void SMTLIB2::parseLetEnd(LExpr* exp)
{
  ...
  LispListReader lRdr(exp->list);
  const vstring& theLetAtom = lRdr.readAtom();
  ASS_EQ(getBuiltInTermSymbol(theLetAtom),TermSymbol::TS_LET);
  ...
  while (bindRdr.hasNext()) {
    ...
    unsigned symbol = 0;
    if (exprSort == (unsigned)Sorts::DefaultSorts::SRT_BOOL) { // it has to be formula term, with atomic formula
...
void SMTLIB2::parseQuantBegin(LExpr* exp)
{
  ...
  _todo.push(make_pair(Parse::SMTLIB2::ParseOperation::PO_PARSE_APPLICATION,exp)); // will create the actual quantified formula and clear the lookup...
  _todo.push(make_pair(Parse::SMTLIB2::ParseOperation::PO_PARSE,lRdr.readNext())); // ... from the only remaining argument, the body
  lRdr.acceptEOL();
}
...
void SMTLIB2::parseAnnotatedTerm(LExpr* exp)
{
  ...
  _todo.push(make_pair(Parse::SMTLIB2::ParseOperation::PO_PARSE,toParse));
}
...
bool SMTLIB2::parseAsSpecConstant(const vstring& id)
{
  ...
  if (StringUtils::isPositiveInteger(id)) {
    ...
    _results.push(ParseResult((unsigned)Sorts::DefaultSorts::SRT_INTEGER,res));
  ...
  if (StringUtils::isPositiveDecimal(id)) {
    ...
    TermList res = TermList(Term::createConstant(symb));
    _results.push(ParseResult((unsigned)Sorts::DefaultSorts::SRT_REAL,res));
...
bool SMTLIB2::parseAsBuiltinFormulaSymbol(const vstring& id, LExpr* exp)
{
  ...
  switch (fs) {
    case Parse::SMTLIB2::FormulaSymbol::FS_TRUE:
      _results.push(ParseResult(Formula::trueFormula()));
      return true;
    case Parse::SMTLIB2::FormulaSymbol::FS_FALSE:
      _results.push(ParseResult(Formula::falseFormula()));
      return true;
    case FormulaSymbol::FS_NOT:
    {
      if (_results.isEmpty() || _results.top().isSeparator()) {
    ...
    case FormulaSymbol::FS_AND:
    case FormulaSymbol::FS_OR:
    {
      ...
      Formula* res;
      if (argcnt > 1) {
        res = new JunctionFormula( (fs==FormulaSymbol::FS_AND) ? Connective::AND : Connective::OR, argLst);
      } else {
        res = argLst->head();
    ...
    case FormulaSymbol::FS_IMPLIES: // done in a right-assoc multiple-argument fashion
    case FormulaSymbol::FS_XOR: // they say XOR should be left-associative, but semantically, it does not matter
    {
      Connective con = (fs== FormulaSymbol::FS_IMPLIES) ? Connective::IMP : Connective::XOR;
    ...
    case FormulaSymbol::FS_GREATER_EQ:
    {
      ...
      if (argLst == nullptr) { // there were only two arguments, let's return lastConjunct
        ...
        Formula* res = new JunctionFormula(Connective::AND, argLst);
        _results.push(ParseResult(res));
      }

      return true;
    }
    case FormulaSymbol::FS_DISTINCT:
    {
      static Stack<TermList> args;
      args.reset();
      ...
      if(args.size()==2) { // if there are 2 just create a disequality
        ...
        res = new JunctionFormula(Connective::AND, diseqs);
      }
    ...
    case FormulaSymbol::FS_IS_INT:
    {
      TermList arg;
      if (_results.isEmpty() || _results.top().isSeparator() ||
          _results.pop().asTerm(arg) != (unsigned)Sorts::DefaultSorts::SRT_REAL) {
        complainAboutArgShortageOrWrongSorts(BUILT_IN_SYMBOL,exp);
      }

      unsigned pred = env.signature->getInterpretingSymbol(Theory::Interpretation::REAL_IS_INT);
    ...
    case FormulaSymbol::FS_EXISTS:
    case FormulaSymbol::FS_FORALL:
    {
      Formula* argFla;
      if (_results.isEmpty() || _results.top().isSeparator() ||
      ...
      Formula* res = new QuantifiedFormula((fs== FormulaSymbol::FS_EXISTS) ? Kernel::Connective::EXISTS : Kernel::Connective::FORALL, qvars, qsorts, argFla);
    ...
    default:
      ASS_EQ(fs,FormulaSymbol::FS_USER_PRED_SYMBOL);
      return false;
  }
}
...
bool SMTLIB2::parseAsBuiltinTermSymbol(const vstring& id, LExpr* exp)
{
  ...
  case TermSymbol::TS_ITE:
    ...
    case TermSymbol::TS_TO_REAL:
    {
      TermList theInt;
      if (_results.isEmpty() || _results.top().isSeparator() ||
          _results.pop().asTerm(theInt) != (unsigned)Sorts::DefaultSorts::SRT_INTEGER) {
        complainAboutArgShortageOrWrongSorts(BUILT_IN_SYMBOL,exp);
      }

      unsigned fun = env.signature->getInterpretingSymbol(Theory::Interpretation::INT_TO_REAL);
      TermList res = TermList(Term::create1(fun,theInt));

      _results.push(ParseResult((unsigned)Sorts::DefaultSorts::SRT_REAL,res));
      return true;
    }
    case TermSymbol::TS_TO_INT:
    {
      TermList theReal;
      if (_results.isEmpty() || _results.top().isSeparator() ||
          _results.pop().asTerm(theReal) != (unsigned)Sorts::DefaultSorts::SRT_REAL) {
        complainAboutArgShortageOrWrongSorts(BUILT_IN_SYMBOL,exp);
      }

      unsigned fun = env.signature->getInterpretingSymbol(Theory::Interpretation::REAL_TO_INT);
      TermList res = TermList(Term::create1(fun,theReal));

      _results.push(ParseResult((unsigned)Sorts::DefaultSorts::SRT_INTEGER,res));
      return true;
    }
    case TermSymbol::TS_SELECT:
    {
      TermList theArray;
      if (_results.isEmpty() || _results.top().isSeparator()) {
        complainAboutArgShortageOrWrongSorts(BUILT_IN_SYMBOL,exp);
      ...
      if (arraySort->getInnerSort() == (unsigned)Sorts::DefaultSorts::SRT_BOOL) {
        OperatorType* predType = Theory::getArrayOperatorType(arraySortIdx,Theory::Interpretation::ARRAY_BOOL_SELECT);
        unsigned pred = env.signature->getInterpretingSymbol(Theory::Interpretation::ARRAY_BOOL_SELECT,predType);

        Formula* res = new AtomicFormula(Literal::create2(pred,true,theArray,theIndex));

        _results.push(ParseResult(res));
      } else {
        OperatorType* funType = Theory::getArrayOperatorType(arraySortIdx, Theory::Interpretation::ARRAY_SELECT);
        unsigned fun = env.signature->getInterpretingSymbol(Theory::Interpretation::ARRAY_SELECT,funType);
    ...
    case TermSymbol::TS_STORE:
    {
      ...
      OperatorType* funType = Theory::getArrayOperatorType(arraySortIdx,Theory::Interpretation::ARRAY_STORE);
      unsigned fun = env.signature->getInterpretingSymbol(Theory::Interpretation::ARRAY_STORE,funType);
    ...
    case Parse::SMTLIB2::TermSymbol::TS_ABS:
    {
      TermList theInt;
      if (_results.isEmpty() || _results.top().isSeparator() ||
          _results.pop().asTerm(theInt) != (unsigned)Sorts::DefaultSorts::SRT_INTEGER) {
      ...
      unsigned fun = env.signature->getInterpretingSymbol(Theory::Interpretation::INT_ABS);
      TermList res = TermList(Term::create1(fun,theInt));
    ...
    case Parse::SMTLIB2::TermSymbol::TS_MOD:
    {
      TermList int1, int2;
      if (_results.isEmpty() || _results.top().isSeparator() || _results.pop().asTerm(int1) != (unsigned)Sorts::DefaultSorts::SRT_INTEGER ||
          _results.isEmpty() || _results.top().isSeparator() || _results.pop().asTerm(int2) != (unsigned)Sorts::DefaultSorts::SRT_INTEGER) {
        complainAboutArgShortageOrWrongSorts(BUILT_IN_SYMBOL,exp);
      }

      unsigned fun = env.signature->getInterpretingSymbol(Theory::Interpretation::INT_REMAINDER_E); // TS_MOD is the always positive remainder, therefore INT_REMAINDER_E
    ...
    case TermSymbol::TS_DIV:
    {
      ...
      if (_results.isEmpty() || _results.top().isSeparator()) {
        if (ts == TermSymbol::TS_MINUS) { // unary minus
    ...
    default:
      ASS_EQ(ts,TermSymbol::TS_USER_FUNCTION);
      return false;
  }
}
...
void SMTLIB2::parseRankedFunctionApplication(LExpr* exp)
{
  ...
  if(headRdr.tryAcceptAtom("divisible")){
    ...
    TermList arg;
    if (_results.isEmpty() || _results.top().isSeparator() ||
        _results.pop().asTerm(arg) != (unsigned)Sorts::DefaultSorts::SRT_INTEGER) {
      complainAboutArgShortageOrWrongSorts("ranked function symbol",exp);
    }

    unsigned pred = env.signature->getInterpretingSymbol(Theory::Interpretation::INT_DIVIDES);
...
SMTLIB2::ParseResult SMTLIB2::parseTermOrFormula(LExpr* body)
{
  ...
  while (_todo.isNonEmpty()) {
    ...
    */
    ...
    switch (op) {
      case Parse::SMTLIB2::ParseOperation::PO_PARSE: {
        if (exp->isList()) {
          LispListReader lRdr(exp->list);

          // schedule arity check
          _results.push(ParseResult()); // separator into results
          _todo.push(make_pair(Parse::SMTLIB2::ParseOperation::PO_CHECK_ARITY,exp)); // check as a todo (exp for error reporting)
          ...
          _todo.push(make_pair(Parse::SMTLIB2::ParseOperation::PO_PARSE_APPLICATION,exp));
          // and all the other arguments too
          while (lRdr.hasNext()) {
            _todo.push(make_pair(Parse::SMTLIB2::ParseOperation::PO_PARSE,lRdr.next()));
          }
      ...
      case Parse::SMTLIB2::ParseOperation::PO_PARSE_APPLICATION: { // the arguments have already been parsed
        vstring id;
      ...
      case Parse::SMTLIB2::ParseOperation::PO_CHECK_ARITY: {
        LOG1("PO_CHECK_ARITY");
      ...
      case Parse::SMTLIB2::ParseOperation::PO_LET_PREPARE_LOOKUP:
        parseLetPrepareLookup(exp);
        continue;
      case Parse::SMTLIB2::ParseOperation::PO_LET_END:
        parseLetEnd(exp);
        continue;
    }
  }
