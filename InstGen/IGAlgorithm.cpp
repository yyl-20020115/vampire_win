...
void IGAlgorithm::init()
{
  ...
  if(_opt.instGenWithResolution()) {
    ...
    if(_opt.globalSubsumption()) {
      _saturationIndexManager->provideIndex(IndexType::GLOBAL_SUBSUMPTION_INDEX, _groundingIndex.ptr());
    }
...
bool IGAlgorithm::addClause(Clause* cl)
{
  ...
  TimeCounter tc(Lib::TimeCounterUnit::TC_INST_GEN_SIMPLIFICATIONS);
...
redundancy_check:
  {
    bool redundant;
    {
      TimeCounter tc2(Lib::TimeCounterUnit::TC_INST_GEN_VARIANT_DETECTION);
...
void IGAlgorithm::processUnprocessed()
{
  ...
  TimeCounter tc(Lib::TimeCounterUnit::TC_INST_GEN_SAT_SOLVING);
  ...
  if(_satSolver->solve()== SATSolver::Status::UNSATISFIABLE) {
...
void IGAlgorithm::tryGeneratingInstances(Clause* cl, unsigned litIdx)
{
  ...
  TimeCounter tc(Lib::TimeCounterUnit::TC_INST_GEN_GEN_INST);
...
void IGAlgorithm::onResolutionClauseDerived(Clause* cl)
{
  ...
  if(_satSolver->solve(true)==SATSolver::Status::UNSATISFIABLE) {
    Clause* foRefutation = getFORefutation(_satSolver->getRefutation(),_satSolver->getRefutationPremiseList());
    throw RefutationFoundException(foRefutation);
  }
}
...
MainLoopResult IGAlgorithm::onModelFound()
{
  ...
  if(_opt.complete(_prb)) {
    MainLoopResult res(Statistics::TerminationReason::SATISFIABLE);
