...
CodeTree::CodeOp CodeTree::CodeOp::getLitEnd(ILStruct* ils)
{
  ...
  CodeOp res;
  res.setAlternative(0);
  res._data=reinterpret_cast<size_t>(ils)|(int)Indexing::CodeTree::InstructionPrefix::LIT_END;
  ASS(res.isLitEnd());
  return res;
}
CodeTree::CodeOp CodeTree::CodeOp::getTermOp(InstructionSuffix i, unsigned num)
{
  ...
  ASS(i== InstructionSuffix::CHECK_FUN || i== InstructionSuffix::CHECK_VAR || i== InstructionSuffix::ASSIGN_VAR);
...
CodeTree::CodeOp CodeTree::CodeOp::getGroundTermCheck(Term* trm)
{
  ...
  CodeOp res;
  res.setAlternative(0);
  res._data=reinterpret_cast<size_t>(trm)|(int)Indexing::CodeTree::InstructionPrefix::CHECK_GROUND_TERM;
...
bool CodeTree::CodeOp::equalsForOpMatching(const CodeOp& o) const
{
  ...
  switch(instrPrefix()) {
  case Indexing::CodeTree::InstructionPrefix::LIT_END:
    return getILS()->equalsForOpMatching(*o.getILS());
  case Indexing::CodeTree::InstructionPrefix::SUCCESS_OR_FAIL:
  case Indexing::CodeTree::InstructionPrefix::CHECK_GROUND_TERM:
    return data()==o.data();
  case Indexing::CodeTree::InstructionPrefix::SUFFIX_INSTR:
    //SEARCH_STRUCT operations in the tree should be handled separately
    //during insertion into the code tree
    ASS_NEQ(instrSuffix(), Indexing::CodeTree::InstructionSuffix::SEARCH_STRUCT);
...
: kind(kind)
{
  ...
  landingOp.setLongInstr(Indexing::CodeTree::InstructionSuffix::SEARCH_STRUCT);
}
...
void CodeTree::SearchStruct::destroy()
{
  ...
  switch(kind) {
  case Indexing::CodeTree::SearchStruct::Kind::FN_STRUCT:
    delete static_cast<FnSearchStruct*>(this);
    break;
  case Indexing::CodeTree::SearchStruct::Kind::GROUND_TERM_STRUCT:
...
CodeTree::CodeOp* CodeTree::SearchStruct::getTargetOp(const FlatTerm::Entry* ftPos)
{
  ...
  case Kind::GROUND_TERM_STRUCT:
    ftPos++;
    ASS_EQ(ftPos->tag(), FlatTerm::EntryTag::FUN_TERM_PTR);
...
: FixedSearchStruct(Indexing::CodeTree::SearchStruct::Kind::GROUND_TERM_STRUCT, length)
{
...
: FixedSearchStruct(Indexing::CodeTree::SearchStruct::Kind::FN_STRUCT, length)
{
...
{
  ASS_EQ(op->instrPrefix(), InstructionPrefix::CHECK_GROUND_TERM);
  ...
  fte++;
  ASS_EQ(fte->tag(), FlatTerm::EntryTag::FUN_TERM_PTR);
  ASS(fte->ptr());
  if(trm!=fte->ptr()) {
    return false;
  }
  fte++;
  ASS_EQ(fte->tag(), FlatTerm::EntryTag::FUN_RIGHT_OFS);
  tp+=fte->number();
  return true;
}
...
void CodeTree::compileTerm(Term* trm, CodeStack& code, CompileContext& cctx, bool addLitEnd)
{
  ...
  if(GROUND_TERM_CHECK && trm->ground()) {
    ...
    if(trm->isLiteral()) {
      ...
      code.push(CodeOp::getTermOp(InstructionSuffix::CHECK_FUN, lit->header()));
    }
    else {
      code.push(CodeOp::getTermOp(InstructionSuffix::CHECK_FUN, trm->functor()));
    }
	...
	if(cctx.varMap.getValuePtr(var,varNumPtr)) {
	  *varNumPtr=cctx.nextVarNum++;
	  code.push(CodeOp::getTermOp(Indexing::CodeTree::InstructionSuffix::ASSIGN_VAR, *varNumPtr));
	  ...
	  code.push(CodeOp::getTermOp(Indexing::CodeTree::InstructionSuffix::CHECK_VAR, *varNumPtr));
	}
      }
      else {
	ASS(s.isTerm());
	...
	if(GROUND_TERM_CHECK && t->ground()) {
	  ...
	  code.push(CodeOp::getTermOp(Indexing::CodeTree::InstructionSuffix::CHECK_FUN, s.term()->functor()));
	}
      }
    }
  }
      ...
      for (;;) {
        ...
        if (treeOp->isCheckFun()) {
          ...
          if (checkFunOps > checkFunOpThreshold) {
            ...
            compressCheckOps(chainStart, SearchStruct::Kind::FN_STRUCT);
            treeOp = chainStart;
            checkFunOps = 0;
        ...
        if (treeOp->isCheckGroundTerm()) {
          ...
          if (checkGroundTermOps > checkGroundTermOpThreshold) {
            ...
            //restart with the chain
            compressCheckOps(chainStart, SearchStruct::Kind::GROUND_TERM_STRUCT);
            treeOp = chainStart;
            checkFunOps = 0;
            checkGroundTermOps = 0;
            continue;
...
void CodeTree::compressCheckOps(CodeOp* chainStart, SearchStruct::Kind kind)
{
  ...
  while (toDo.isNonEmpty()) {
    ...
    if ((kind == SearchStruct::Kind::FN_STRUCT && op->isCheckFun()) ||
            (kind == SearchStruct::Kind::GROUND_TERM_STRUCT && op->isCheckGroundTerm())) {
      chfOps.push(op);
  ...
  SearchStruct* ss;
  if(kind==SearchStruct::Kind::FN_STRUCT) {
    ...
    ASS_EQ(kind, SearchStruct::Kind::GROUND_TERM_STRUCT);
...
void CodeTree::optimizeMemoryAfterRemoval(Stack<CodeOp*>* firstsInBlocks, CodeOp* removedOp)
{
  ...
  for(;;) {
	...
	return;
      ...
      *tgtPtr=alt;
      if(alt) {
	ASS( (ss->kind==SearchStruct::Kind::FN_STRUCT && alt->isCheckFun()) ||
	    (ss->kind==SearchStruct::Kind::GROUND_TERM_STRUCT && alt->isCheckGroundTerm()) );
	return;
      }
      for(size_t i=0; i<ss->length; i++) {
	if(ss->targets[i]!=0) {
...
bool CodeTree::RemovingMatcher::next()
{
  ...
  for(;;) {
    ...
    case Indexing::CodeTree::InstructionPrefix::SUFFIX_INSTR:
      ...
      case Indexing::CodeTree::InstructionSuffix::CHECK_FUN:
        shouldBacktrack=!doCheckFun();
        break;
      case Indexing::CodeTree::InstructionSuffix::ASSIGN_VAR:
        shouldBacktrack=!doAssignVar();
        break;
      case Indexing::CodeTree::InstructionSuffix::CHECK_VAR:
        shouldBacktrack=!doCheckVar();
        break;
      case Indexing::CodeTree::InstructionSuffix::SEARCH_STRUCT:
        if(doSearchStruct()) {
...
inline bool CodeTree::RemovingMatcher::doSearchStruct()
{
  ...
  ASS_EQ(op->instrSuffix(), InstructionSuffix::SEARCH_STRUCT);
...
{
  ASS_EQ(op->instrSuffix(), InstructionSuffix::CHECK_FUN);
...
{
  ASS_EQ(op->instrSuffix(), InstructionSuffix::ASSIGN_VAR);

  //we are looking for variants and they match only other variables into variables
  unsigned var=op->arg();
  const FlatTerm::Entry* fte=&(*ft)[tp];
  if(fte->tag()!=FlatTerm::EntryTag::VAR) {
    return false;
  }
...
{
  ASS_EQ(op->instrSuffix(), InstructionSuffix::CHECK_VAR);

  //we are looking for variants and they match only other variables into variables
  unsigned var=op->arg();
  const FlatTerm::Entry* fte=&(*ft)[tp];
  if(fte->tag()!= FlatTerm::EntryTag::VAR || bindings[var]!=fte->number()) {
    return false;
  }
  tp++;
  return true;
...
inline void CodeTree::Matcher::doAssignVar()
{
  ...
  if(fte->tag()== FlatTerm::EntryTag::VAR) {
    ...
    ASS_EQ(fte->tag(), FlatTerm::EntryTag::FUN);
    fte++;
    ASS_EQ(fte->tag(), FlatTerm::EntryTag::FUN_TERM_PTR);
    ASS(fte->ptr());
