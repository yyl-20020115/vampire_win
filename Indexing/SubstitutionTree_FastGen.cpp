...
main_loop_start:
    ...
    if(curr) {
      if(sibilingsRemain) {
	ASS(_nodeTypes.top()!=NodeAlgorithm::UNSORTED_LIST || *static_cast<Node**>(_alternatives.top()));
	...
	continue;
      ...
      if(parentType==Indexing::SubstitutionTree::NodeAlgorithm::UNSORTED_LIST) {
	...
	ASS_EQ(parentType,NodeAlgorithm::SKIP_LIST)
    ...
    while(!curr->isLeaf() && curr->algorithm()==Indexing::SubstitutionTree::NodeAlgorithm::UNSORTED_LIST && static_cast<UArrIntermediateNode*>(curr)->_size==1) {
...
bool SubstitutionTree::FastGeneralizationsIterator::enterNode(Node*& curr)
{
  ...
  if(currType==Indexing::SubstitutionTree::NodeAlgorithm::UNSORTED_LIST) {
    ...
    NodeList* nl;
    ASS_EQ(currType, NodeAlgorithm::SKIP_LIST);
