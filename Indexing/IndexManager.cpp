...
IndexManager::~IndexManager()
{
  ...
  if(_alg) {
    release(Indexing::IndexType::GENERATING_SUBST_TREE);
  }
}
...
void IndexManager::attach(SaturationAlgorithm* salg)
{
  ...
  request(Indexing::IndexType::GENERATING_SUBST_TREE);
}
...
void IndexManager::release(IndexType t)
{
  ...
  e.refCnt--;
  if(e.refCnt==0) {
    if(t==Indexing::IndexType::GENERATING_SUBST_TREE) {
      _genLitIndex=0;
    }
    delete e.index;
    _store.remove(t);
...
Index* IndexManager::create(IndexType t)
{
  ...
  switch(t) {
  case Indexing::IndexType::GENERATING_SUBST_TREE:
    ...
    isGenerating = true;
    break;
  case Indexing::IndexType::SIMPLIFYING_SUBST_TREE:
    is=new LiteralSubstitutionTree();
    res=new SimplifyingLiteralIndex(is);
    isGenerating = false;
    break;

  case Indexing::IndexType::SIMPLIFYING_UNIT_CLAUSE_SUBST_TREE:
    is=new LiteralSubstitutionTree();
    res=new UnitClauseLiteralIndex(is);
    isGenerating = false;
    break;
  case Indexing::IndexType::GENERATING_UNIT_CLAUSE_SUBST_TREE:
    is=new LiteralSubstitutionTree();
    res=new UnitClauseLiteralIndex(is);
    isGenerating = true;
    break;
  case Indexing::IndexType::GENERATING_NON_UNIT_CLAUSE_SUBST_TREE:
    is=new LiteralSubstitutionTree();
    res=new NonUnitClauseLiteralIndex(is);
    isGenerating = true;
    break;

  case Indexing::IndexType::SUPERPOSITION_SUBTERM_SUBST_TREE:
    ...
    isGenerating = true;
    break;
  case Indexing::IndexType::SUPERPOSITION_LHS_SUBST_TREE:
    tis=new TermSubstitutionTree(useConstraints);
    res=new SuperpositionLHSIndex(tis, _alg->getOrdering(), _alg->getOptions());
    isGenerating = true;
    break;

  case Indexing::IndexType::ACYCLICITY_INDEX:
    tis = new TermSubstitutionTree();
    res = new AcyclicityIndex(tis);
    isGenerating = true;
    break;

  case Indexing::IndexType::DEMODULATION_SUBTERM_SUBST_TREE:
    tis=new TermSubstitutionTree();
    res = 0;
    //TODO:
    //res=new DemodulationSubtdermIndex(tis);
    isGenerating = false;
    break;
  case Indexing::IndexType::DEMODULATION_LHS_SUBST_TREE:
//    tis=new TermSubstitutionTree();
    tis=new CodeTreeTIS();
  ...
  case Indexing::IndexType::FW_SUBSUMPTION_CODE_TREE:
    res=new CodeTreeSubsumptionIndex();
    isGenerating = false;
    break;

  case Indexing::IndexType::FW_SUBSUMPTION_SUBST_TREE:
    is=new LiteralSubstitutionTree();
//    is=new CodeTreeLIS();
  ...
  case Indexing::IndexType::REWRITE_RULE_SUBST_TREE:
    is=new LiteralSubstitutionTree();
    res=new RewriteRuleIndex(is, _alg->getOrdering());
    isGenerating = false;
    break;

  case Indexing::IndexType::GLOBAL_SUBSUMPTION_INDEX:
