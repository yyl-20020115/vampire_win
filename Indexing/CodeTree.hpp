...
public:
  ...
  struct CodeOp
  {
     ...
     */
    inline bool isSuccess() const { return instrPrefix()== InstructionPrefix::SUCCESS_OR_FAIL && data(); }
    inline bool isFail() const { return !data(); }
    inline bool isLitEnd() const { return instrPrefix()== InstructionPrefix::LIT_END; }
    inline bool isSearchStruct() const { return instrPrefix()== InstructionPrefix::SUFFIX_INSTR && instrSuffix()== InstructionSuffix::SEARCH_STRUCT; }
    inline bool isCheckFun() const { return instrPrefix()== InstructionPrefix::SUFFIX_INSTR && instrSuffix()== InstructionSuffix::CHECK_FUN; }
    inline bool isCheckGroundTerm() const { return instrPrefix()== InstructionPrefix::CHECK_GROUND_TERM; }

    inline Term* getTargetTerm() const
    {
      ASS(isCheckGroundTerm());
      return reinterpret_cast<Term*>(data()&~static_cast<size_t>(InstructionPrefix::CHECK_GROUND_TERM));
    }
    ...
    {
      ASS(isLitEnd());
      return reinterpret_cast<ILStruct*>(data()&~static_cast<size_t>(InstructionPrefix::LIT_END));
    }
    ...
    {
      ASS_EQ(instrPrefix(), InstructionPrefix::SUFFIX_INSTR);
    ...
    inline void setLongInstr(InstructionSuffix i) { _info.prefix= (unsigned)InstructionPrefix::SUFFIX_INSTR; _info.suffix=(unsigned)i; }
