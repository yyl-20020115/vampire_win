...
vstring Formula::toString (Connective c)
{
  static vstring names [] =
    { "", "&", "|", "=>", "<=>", "<~>", "~", "!", "?", "$var", "$false", "$true", "",""};
  ASS_EQ(sizeof(names)/sizeof(vstring), ((unsigned)Connective::NOCONN+1));
...
vstring Formula::toString(const Formula* formula)
{
  ...
  Stack<Todo> stack;
  stack.push({false,Connective::NOCONN,formula});
  ...
  while (stack.isNonEmpty()) {
    ...
    if (todo.wrapInParenthesis) {
      res += "(";
      stack.push({false,Connective::NOCONN,nullptr}); // render the final closing bracket
    }
    ...
    case Connective::OR:
      {
        ...
        while (FormulaList::isNonEmpty(fs)) {
          ...
          stack.push({arg->parenthesesRequired(c),FormulaList::isNonEmpty(fs) ? c : Connective::NOCONN,arg});
        }
    ...
    case Connective::XOR:
      ...
      stack.push({f->left()->parenthesesRequired(c),Connective::NOCONN,f->left()}); // first argument without con
    ...
    case Connective::NOT:
      {
        ...
        stack.push({arg->parenthesesRequired(c),Connective::NOCONN,arg});
    ...
    case Connective::EXISTS:
      {
        ...
        while (vs.hasNext()) {
          ...
          if (hasSorts) {
            ASS(ss.hasNext());
            t = ss.next();
            if (t != (unsigned)Sorts::DefaultSorts::SRT_DEFAULT) {
              res += " : " + env.sorts->sortName(t);
            }
          } else if (SortHelper::tryGetVariableSort(var, const_cast<Formula*>(f),t) && t != (unsigned)Sorts::DefaultSorts::SRT_DEFAULT) {
            res += " : " + env.sorts->sortName(t);
          }
          first = false;
        }
...
Formula::VarList* Formula::boundVariables () const
{
  ...
  while(sfit.hasNext()) {
    ...
    if(sf->connective() == Connective::FORALL || sf->connective() == Connective::EXISTS) {
...
void Formula::collectAtoms(Stack<Literal*>& acc)
{
  ...
  while(sfit.hasNext()) {
    Formula* sf = sfit.next();
    if(sf->connective()== Connective::LITERAL) {
...
Formula* Formula::createITE(Formula* condition, Formula* thenArg, Formula* elseArg)
{
  ...
  TermList iteTerm(Term::createITE(condition, thenTerm, elseTerm, (unsigned)Sorts::DefaultSorts::SRT_BOOL));
...
Formula* Formula::createLet(unsigned functor, Formula::VarList* variables, TermList body, Formula* contents)
{
  ...
  TermList letTerm(Term::createLet(functor, variables, body, contentsTerm, (unsigned)Sorts::DefaultSorts::SRT_BOOL));
...
Formula* Formula::createLet(unsigned predicate, Formula::VarList* variables, Formula* body, Formula* contents)
{
  ...
  TermList contentsTerm(Term::createFormula(contents));
  TermList letTerm(Term::createLet(predicate, variables, bodyTerm, contentsTerm, (unsigned)Sorts::DefaultSorts::SRT_BOOL));
  return new BoolTermFormula(letTerm);
}
...
Formula* Formula::quantify(Formula* f)
{
  ...
  if(varLst) {
    ...
    f=new QuantifiedFormula(Connective::FORALL, varLst, 0, f);
  }
  return f;
}
...
Formula* Formula::fromClause(Clause* cl)
{
  ...
  Formula* res=JunctionFormula::generalJunction(Connective::OR, resLst);
  ...
  if(varLst) {
    //TODO could compute the sorts list, but don't want to!
    res=new QuantifiedFormula(Connective::FORALL, varLst, 0, res);
  }
