...
bool SortHelper::getResultSortOrMasterVariable(const Term* t, unsigned& resultSort, TermList& resultVar)
{
  ...
  switch(t->functor()) {
    ...
    case Term::SF_FORMULA:
      resultSort = (unsigned)Sorts::DefaultSorts::SRT_BOOL;
      return true;
    case Term::SF_TUPLE: {
  ...
  while (sfit.hasNext()) {
    ...
    if(sf->connective() == Connective::BOOL_TERM){
      ...
      if(stt.isVar() && stt.var()==var){
        res = (unsigned)Sorts::DefaultSorts::SRT_BOOL;
        return true;
      }
      if(stt.isTerm()){
        Term* st = stt.term();
...
void SortHelper::collectVariableSortsIter(CollectTask task, DHMap<unsigned,unsigned>& map)
{
  ...
  while (todo.isNonEmpty()) {
    ...
    switch(task.fncTag) {
    case CollectWhat::COLLECT_TERM: {
        Term* term = task.t;

        unsigned position = 0;
        for (TermList* ts = term->args(); ts->isNonEmpty(); ts = ts->next()) {
          CollectTask newTask;
          newTask.fncTag = CollectWhat::COLLECT_TERMLIST;
          newTask.ts = *ts;
      ...
      case CollectWhat::COLLECT_TERMLIST: {
        TermList ts = task.ts;
        ...
        if (ts.isTerm()) {
          ...
          if (term->isSpecial()) {
            newTask.fncTag = CollectWhat::COLLECT_SPECIALTERM;
            todo.push(newTask);
          } else {
            newTask.fncTag = CollectWhat::COLLECT_TERM;
            todo.push(newTask);
          }
      ...
      case CollectWhat::COLLECT_SPECIALTERM: {
        Term* term = task.t;
        ...
        switch (term->functor()) {
          case Term::SF_ITE: {
            ...
            newTask.fncTag = CollectWhat::COLLECT_FORMULA;
      ...
      case CollectWhat::COLLECT_FORMULA: {
        Formula* f = task.f;
        ...
        while (sfit.hasNext()) {
          ...
          switch (sf->connective()) {
            ...
            case Connective::BOOL_TERM: {
              ...
              if (ts.isVar()) {
                if (!map.insert(ts.var(), (unsigned)Sorts::DefaultSorts::SRT_BOOL)) {
                  ASS_EQ((unsigned)Sorts::DefaultSorts::SRT_BOOL, map.get(ts.var()));
                }
              } else {
                ...
                newTask.fncTag = CollectWhat::COLLECT_SPECIALTERM;
                newTask.t = ts.term();
                newTask.contextSort = (unsigned)Sorts::DefaultSorts::SRT_BOOL;
...
void SortHelper::collectVariableSorts(Term* term, DHMap<unsigned,unsigned>& map)
{
  ...
  CollectTask t;
  t.fncTag = CollectWhat::COLLECT_TERM;
  t.t = term;
...
void SortHelper::collectVariableSorts(TermList ts, unsigned contextSort, DHMap<unsigned,unsigned>& map)
{
  ...
  CollectTask t;
  t.fncTag = CollectWhat::COLLECT_TERMLIST;
  t.ts = ts;
...
void SortHelper::collectVariableSortsSpecialTerm(Term* term, unsigned contextSort, DHMap<unsigned,unsigned>& map) {
  ...
  CollectTask task;
  task.fncTag = CollectWhat::COLLECT_SPECIALTERM;
  task.t = term;
...
void SortHelper::collectVariableSorts(Formula* f, DHMap<unsigned,unsigned>& map)
{
  ...
  CollectTask task;
  task.fncTag = CollectWhat::COLLECT_FORMULA;
  task.f = f;
...
void SortHelper::collectVariableSorts(Unit* u, DHMap<unsigned,unsigned>& map)
{
  ...
  while (cit.hasNext()) {
    ...
    CollectTask task;
    task.fncTag = CollectWhat::COLLECT_TERM;
    task.t = l;
