...
FlatTerm* FlatTerm::create(Term* t)
{
  ...
  size_t fti=0;
  res->_data[fti++]=Entry(EntryTag::FUN,
      t->isLiteral() ? static_cast<Literal*>(t)->header() : t->functor());
  res->_data[fti++]=Entry(t);
  res->_data[fti++]=Entry(EntryTag::FUN_RIGHT_OFS, getEntryCount(t));
  ...
  while(sti.hasNext()) {
    ...
    if(s.isVar()) {
      ASS(s.isOrdinaryVar());
      res->_data[fti++]=Entry(EntryTag::VAR, s.var());
    }
    else {
      ASS(s.isTerm());
      res->_data[fti++]=Entry(EntryTag::FUN, s.term()->functor());
      res->_data[fti++]=Entry(s.term());
      res->_data[fti++]=Entry(EntryTag::FUN_RIGHT_OFS, getEntryCount(s.term()));
    }
  }
  ASS_EQ(fti, entries);
...
FlatTerm* FlatTerm::create(TermList t)
{
  ...
  res->_data[0]=Entry(EntryTag::VAR, t.var());
...
void FlatTerm::swapCommutativePredicateArguments()
{
  ...
  ASS_EQ((*this)[0].tag(), EntryTag::FUN);
  ASS_EQ((*this)[0].number()|1, 1); //as for now, the only commutative predicate is equality

  size_t firstStart=3;
  size_t firstLen;
  if((*this)[firstStart].tag()== EntryTag::FUN) {
    ASS_EQ((*this)[firstStart+2].tag(), EntryTag::FUN_RIGHT_OFS);
    firstLen=(*this)[firstStart+2].number();
  }
  else {
    ASS_EQ((*this)[firstStart].tag(), EntryTag::VAR);
    firstLen=1;
  }
  ...
  if((*this)[secStart].tag()==EntryTag::FUN) {
    ASS_EQ((*this)[secStart+2].tag(), EntryTag::FUN_RIGHT_OFS);
    secLen=(*this)[secStart+2].number();
  }
  else {
    ASS_EQ((*this)[secStart].tag(), EntryTag::VAR);
    secLen=1;
  }
