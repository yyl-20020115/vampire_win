...
void Problem::addUnits(UnitList* newUnits)
{
  ...
  if(_propertyValid) {
    TimeCounter tc(Lib::TimeCounterUnit::TC_PROPERTY_EVALUATION);
...
void Problem::refreshProperty() const
{
  ...
  TimeCounter tc(Lib::TimeCounterUnit::TC_PROPERTY_EVALUATION);
  ScopedLet<Statistics::ExecutionPhase> phaseLet(env.statistics->phase, Statistics::ExecutionPhase::PROPERTY_SCANNING);
...
void Problem::invalidateEverything()
{
  ...
  invalidateProperty();
  _hasFormulas = MaybeBool::Value::_UNKNOWN;
  _hasEquality = MaybeBool::Value::_UNKNOWN;
  _hasInterpretedOperations = MaybeBool::Value::_UNKNOWN;
  _hasFOOL = MaybeBool::Value::_UNKNOWN;
  _hasInterpretedEquality = MaybeBool::Value::_UNKNOWN;
