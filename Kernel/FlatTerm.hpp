...
public:
  ...
  {
    Entry() {}
    Entry(EntryTag tag, unsigned num) { _info.tag=(unsigned)tag; _info.number=num; }
    Entry(Term* ptr) : _ptr(ptr) { ASS_EQ(tag(), EntryTag::FUN_TERM_PTR); }
    ...
    inline bool isVar() const { return tag()==FlatTerm::EntryTag::VAR; }
    inline bool isVar(unsigned num) const { return isVar() && number()==num; }
    inline bool isFun() const { return tag()== FlatTerm::EntryTag::FUN; }
