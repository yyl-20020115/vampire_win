...
Clause::Clause(unsigned length,const Inference& inf)
  : Unit(Kernel::Unit::Kind::CLAUSE,inf),
    _length(length),
    _color((unsigned)Color::COLOR_INVALID),
    _extensionality(false),
    _extensionalityTag(false),
    _component(false),
    _store(Kernel::Clause::Store::NONE),
    _numSelected(0),
    _weight(0),
...
{
  return (_store == Kernel::Clause::Store::NONE) && _refCnt == 0 &&
    !isFromPreprocessing();
}
...
void Clause::setStore(Store s)
{
  ...
  if (_store==Kernel::Clause::Store::SELECTED) {
    ASS_EQ(selected, this);
    selected=0;
  }
  if (s==Kernel::Clause::Store::SELECTED) {
    ASS_EQ(selected, 0);
    selected=this;
  }
#endif
...
vstring Clause::toString() const
{
  ...
  if(env.options->proofExtra()!=Options::ProofExtra::OFF){
    ...
    if (env.colorUsed) {
      result += vstring(",col:") + Int::toString((unsigned)color());
    }
...
void Clause::computeColor() const
{
  ...
  ASS_EQ(_color, (unsigned)Color::COLOR_INVALID);

  Color color = Color::COLOR_TRANSPARENT;

  if (env.colorUsed) {
    unsigned clen=length();
    for(unsigned i=0;i<clen;i++) {
      color = static_cast<Color>((unsigned)color | (unsigned)(*this)[i]->color());
    }
    ASS_L(color, Color::COLOR_INVALID);
  }

  _color=(unsigned)color;
}
