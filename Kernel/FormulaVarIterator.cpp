...
FormulaVarIterator::FormulaVarIterator(const Formula* f)
  : _found(false)
{
  _instructions.push(Kernel::FormulaVarIterator::Instruction::FVI_FORMULA);
  _formulas.push(f);
  ...
  : _found(false)
{
  ...
  _instructions.push(Instruction::FVI_TERM);
  _terms.push(t);
} // FormulaVarIterator::FormulaVarIterator
  ...
  : _found(false)
{
  ...
  _instructions.push(Instruction::FVI_TERM_LIST);
  _termLists.push(*ts);
} // FormulaVarIterator::FormulaVarIterator
    ...
    case Instruction::FVI_FORMULA: {
        ...
        switch (f->connective()) {
          ...
          case Kernel::Connective::OR: {
            ...
            while (fs.hasNext()) {
              _instructions.push(Instruction::FVI_FORMULA);
          ...
          case Kernel::Connective::EXISTS:
            _instructions.push(Instruction::FVI_UNBIND);
            _instructions.push(Instruction::FVI_FORMULA);
            _formulas.push(f->qarg());
            _instructions.push(Instruction::FVI_BIND);
            _vars.push(f->vars());
            break;

          case Kernel::Connective::BOOL_TERM:
            _instructions.push(Kernel::FormulaVarIterator::Instruction::FVI_TERM_LIST);
            _termLists.push(f->getBooleanTerm());
            break;

          case Kernel::Connective::_TRUE:
          case Kernel::Connective::_FALSE:
          case Kernel::Connective::NAME:
            break;
          default:
            ASSERTION_VIOLATION;
        }
      ...
      case Kernel::FormulaVarIterator::Instruction::FVI_TERM: {
      ...
      case Instruction::FVI_TERM_LIST: {
        ...
        if (ts.isVar()) {
          ...
          _instructions.push(Kernel::FormulaVarIterator::Instruction::FVI_TERM);
          _terms.push(ts.term());
        }
        break;
      }

      case Instruction::FVI_BIND: {
