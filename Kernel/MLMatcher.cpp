...
struct MatchingData {
  ...
  InitResult ensureInit(unsigned bIndex)
  {
    ...
    if(!isInitialized(bIndex)) {
      ...
      if(altCnt==0) {
	return InitResult::NO_ALTERNATIVE;
      }
	...
	pair<int,int>* iinfo=getIntersectInfo(pbi, bIndex);
      ...
      if(bIndex>0 && remAlts==0) {
        return InitResult::MUST_BACKTRACK;
      }
    }
...
binding_start:
  while(true) {
    ...
    if(ires!=MatchingData::InitResult::OK) {
      if(ires==MatchingData::InitResult::MUST_BACKTRACK) {
	currBLit--;
	continue;
      } else {
	ASS_EQ(ires,MatchingData::InitResult::NO_ALTERNATIVE);
	return false;
      }
    }
