...
void Term::destroyNonShared()
{
  ...
  for(;;) {
    if (ts->tag()== TermTag::REF && !ts->term()->shared()) {
    ...
    switch(functor()) {
      ...
      case Term::SF_LET_TUPLE: {
        ...
        for (unsigned i = 0; i < IntList::length(symbols); i++) {
          Signature::Symbol* symbol = (fnType->arg(i) == (unsigned)Sorts::DefaultSorts::SRT_BOOL)
...
vstring Literal::toString() const
{
  ...
  if (isEquality()) {
    ...
    if (SortHelper::getEqualityArgumentSort(this) ==(unsigned) Sorts::DefaultSorts::SRT_BOOL) {
      res = "("+res+")";
    }
...
Term* Term::createTupleLet(unsigned tupleFunctor, IntList* symbols, TermList binding, TermList body, unsigned bodySort)
{
  ...
  while (sit.hasNext()) {
    ...
    bool isPredicate = tupleSymbol->fnType()->arg(arg) ==(unsigned) Sorts::DefaultSorts::SRT_BOOL;
   ...
   _vars(0)
{
  ...
  _args[0]._info.tag = (unsigned)TermTag::FUN;
  _args[0]._info.distinctVars = TERM_DIST_VAR_UNKNOWN;
#if USE_MATCH_TAG
  matchTag().makeEmpty();
#endif
...
void Term::assertValid() const
{
  ...
  ASS_EQ(_args[0]._info.tag, (unsigned)TermTag::FUN);
}
...
void TermList::assertValid() const
{
  if (this->isTerm()) {
    ...
    ASS_EQ(_term->_args[0]._info.tag, (unsigned)TermTag::FUN);
  }
}
