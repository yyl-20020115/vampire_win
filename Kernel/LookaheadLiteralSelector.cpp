...
struct LookaheadLiteralSelector::GenIteratorIterator
{
  ...
  start:
    ...
    {
      if(!imgr->contains(Indexing::IndexType::SUPERPOSITION_SUBTERM_SUBST_TREE)) { stage++; goto start; }
      TermIndex* bsi=static_cast<TermIndex*>(imgr->get(Indexing::IndexType::SUPERPOSITION_SUBTERM_SUBST_TREE));
      ASS(bsi);
    ...
    {
      if(!imgr->contains(Indexing::IndexType::SUPERPOSITION_LHS_SUBST_TREE)) { stage++; goto start; }
      TermIndex* fsi=static_cast<TermIndex*>(imgr->get(Indexing::IndexType::SUPERPOSITION_LHS_SUBST_TREE));
      ASS(fsi);
...
Literal* LookaheadLiteralSelector::pickTheBest(Literal** lits, unsigned cnt)
{
  ...
  if(candidates.isNonEmpty()) {
    ...
    while(candidates.isNonEmpty()) {
      ...
      if(comp.compare(res, lit)==Lib::Comparison::LESS) {
	res=lit;
      }
    }
  }
