...
Sorts::Sorts()
{
  ...
  aux = addSort("$i",true);
  ASS_EQ(aux, (unsigned)Sorts::DefaultSorts::SRT_DEFAULT);

  aux = addSort("$o",true);
  ASS_EQ(aux, (unsigned)Sorts::DefaultSorts::SRT_BOOL);

  aux = addSort("$int",true);
  ASS_EQ(aux, (unsigned)Sorts::DefaultSorts::SRT_INTEGER);

  aux = addSort("$rat",true);
  ASS_EQ(aux, (unsigned)Sorts::DefaultSorts::SRT_RATIONAL);

  aux = addSort("$real",true);
  ASS_EQ(aux, (unsigned)Sorts::DefaultSorts::SRT_REAL);
...
{
  CALL("Sorts::sortName");
  if (env.options->showFOOL() && idx == (unsigned)Sorts::DefaultSorts::SRT_BOOL) {
...
OperatorType::OperatorKey* OperatorType::setupKey(unsigned arity, const unsigned* sorts)
{
  ...
  if (!sorts) {
    ...
    for (unsigned i = 0; i < arity; i++) {
      (*key)[i] = (unsigned)Sorts::DefaultSorts::SRT_DEFAULT;
    }
  } else {
