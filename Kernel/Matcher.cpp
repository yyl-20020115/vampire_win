...
bool OCMatchIterator::occursCheck()
{
  ...
  while(bit.hasNext()) {
    ...
    if(!statuses.getValuePtr(var0, pst0)) {
      ASS_EQ(*pst0, OCStatus::CHECKED);
      continue;
    }

    *pst0= OCStatus::ENQUEUED;
    toDo.push(var0);
	...
	ASS_EQ(statuses.get(var), OCStatus::TRAVERSING);
	statuses.set(var, OCStatus::CHECKED);
	continue;
      }

      unsigned var=task;

      ASS_EQ(statuses.get(var), OCStatus::ENQUEUED);
      statuses.set(var, OCStatus::TRAVERSING);
...
//      VariableIterator vit(tgt.term());
	...
	if(!statuses.getValuePtr(chvar, pChStatus)) {
	  if(*pChStatus== OCStatus::TRAVERSING) {
	    return false;
	  }
	  ASS_REP(*pChStatus== OCStatus::CHECKED||*pChStatus== OCStatus::ENQUEUED, *pChStatus);
	  continue;
	}
	*pChStatus= OCStatus::ENQUEUED;
...
public:
  ...
  {
    if(_state!= State::FINISHED && _state!= State::FIRST) {
	backtrack();
    }
    ASS(_bdata.isEmpty());
  }
  bool hasNext()
  {
    ...
    switch(_state) {
    case State::NEXT_STRAIGHT:
	if(_matcher->matchArgs(_base,_instance)) {
	  _state= State::NEXT_REVERSED;
	  break;
	}
	//no break here intentionally
    case State::NEXT_REVERSED:
	if(_matcher->matchReversedArgs(_base,_instance)) {
	  _state= State::NEXT_CLEANUP;
	  break;
	}
    //no break here intentionally
    case State::NEXT_CLEANUP:
    ...
    ASS(_state!= State::FINISHED || _bdata.isEmpty());
