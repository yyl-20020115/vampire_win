...
Ordering::Result Ordering::fromComparison(Comparison c)
{
  ...
  switch(c) {
  case Lib::Comparison::GREATER:
    return Kernel::Ordering::Result::GREATER;
  case Lib::Comparison::EQUAL:
    return Kernel::Ordering::Result::EQUAL;
  case Lib::Comparison::LESS:
...
void Ordering::removeNonMaximal(LiteralList*& lits) const
{
  ...
  while(*ptr1) {
    ...
    while(*ptr2 && *ptr1) {
      ...
      if(res==Ordering::Result::GREATER || res== Ordering::Result::GREATER_EQ || res== Ordering::Result::EQUAL) {
	LiteralList::pop(*ptr2);
	continue;
      } else if(res== Ordering::Result::LESS || res== Ordering::Result::LESS_EQ) {
...
Ordering::Result Ordering::getEqualityArgumentOrder(Literal* eq) const
{
  ...
  if((int)precomputed!=0) {
    ...
    eq->setArgumentOrderValue((int)res);
  }
  return res;
}
...
Ordering::Result PrecedenceOrdering::compare(Literal* l1, Literal* l2) const
{
  ...
  if( (l1->isNegative() ^ l2->isNegative()) && (p1==p2) &&
    ...
    return l1->isNegative() ? Result::LESS : Result::GREATER;
  }

  if (p1 != p2) {
    Comparison levComp=Int::compare(predicateLevel(p1),predicateLevel(p2));
    if(levComp!=Lib::Comparison::EQUAL) {
...
Comparison PrecedenceOrdering::compareFunctors(unsigned fun1, unsigned fun2) const
{
  ...
  if(fun1==fun2) {
    return Lib::Comparison::EQUAL;
  }
  switch(compareFunctionPrecedences(fun1, fun2)) {
  case Kernel::Ordering::Result::GREATER: return Lib::Comparison::GREATER;
  case Kernel::Ordering::Result::LESS: return Lib::Comparison::LESS;
  default:
    ASSERTION_VIOLATION;
  }
}
  ...
  Comparison compare(unsigned f1, unsigned f2)
  {
    ...
    switch(boost){
      ...
      case Options::SymbolPrecedenceBoost::GOAL:
        if(g1 && !g2){ res = Comparison::GREATER; }
        else if(!g1 && g2){ res = Comparison::LESS; }
        break;
      case Options::SymbolPrecedenceBoost::UNIT:
        if(u1 && !u2){ res = Comparison::GREATER; }
        else if(!u1 && u2){ res = Comparison::LESS; }
        break;
      case Options::SymbolPrecedenceBoost::GOAL_UNIT:
        if(g1 && !g2){ res = Comparison::GREATER; }
        else if(!g1 && g2){ res = Comparison::LESS; }
        else if(u1 && !u2){ res = Comparison::GREATER; }
        else if(!u1 && u2){ res = Comparison::LESS; }
        break;
    }
