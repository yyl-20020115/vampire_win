...
OperatorType* Signature::Symbol::fnType() const
{
  ...
  if (!_type) {
    _type = OperatorType::getFunctionType(arity(), (unsigned*)0, (unsigned)Sorts::DefaultSorts::SRT_DEFAULT);
  }
  return _type;
}
    ...
    _termAlgebras()
{
  ...
  // initialize equality
  addInterpretedPredicate(Theory::Interpretation::EQUAL, OperatorType::getPredicateType(2), "=");
...
unsigned Signature::addInterpretedFunction(Interpretation interpretation, OperatorType* type, const vstring& name)
{
  ...
  vstring symbolKey = name+"_i"+Int::toString((int)interpretation)+(Theory::isPolymorphic(interpretation) ? type->toString() : "");
