...
private:
  ...
  {
    if(_posNum>0 && (res== Result::LESS || res== Result::LESS_EQ || res== Result::EQUAL)) {
      res= Result::INCOMPARABLE;
    } else if(_negNum>0 && (res== Result::GREATER || res== Result::GREATER_EQ || res== Result::EQUAL)) {
      res= Result::INCOMPARABLE;
    }
    return res;
  }
...
Ordering::Result KBO::State::result(Term* t1, Term* t2)
{
  Result res;
  if(_weightDiff) {
    res=_weightDiff>0 ? Result::GREATER : Result::LESS;
    ...
    if(t1->isLiteral()) {
      ...
      res=(prec1>prec2)? Result::GREATER: Result::LESS;
    } else {
      res=_kbo.compareFunctionPrecedences(t1->functor(), t2->functor());
      ASS_REP(res== Result::GREATER || res== Result::LESS, res); //precedence ordering must be total
    }
  } else {
    res=_lexResult;
  }
  res=applyVariableCondition(res);
  ASS( !t1->ground() || !t2->ground() || res!= Result::INCOMPARABLE);
...
void KBO::State::traverse(Term* t1, Term* t2)
{
  ...
  ASS(t1->arity());
  ASS_EQ(_lexResult, Result::EQUAL);
  ...
  while(!stack.isEmpty()) {
    ...
    if(ss->isEmpty()) {
      ASS(tt->isEmpty());
      depth--;
      ASS_NEQ(_lexResult, Result::EQUAL);
      if(_lexResult!= Result::EQUAL && depth<lexValidDepth) {
	lexValidDepth=depth;
	if(_weightDiff!=0) {
	  _lexResult=_weightDiff>0 ? Result::GREATER : Result::LESS;
	}
    ...
    if(TermList::sameTopFunctor(*ss,*tt)) {
      ...
      traverse(*ss,1);
      traverse(*tt,-1);
      if(_lexResult== Result::EQUAL) {
	_lexResult=innerResult(*ss, *tt);
	lexValidDepth=depth;
	ASS(_lexResult!= Result::EQUAL);
	ASS(_lexResult!= Result::GREATER_EQ);
	ASS(_lexResult!= Result::LESS_EQ);
      }
    }
  }
  ASS_EQ(depth,0);
...
Ordering::Result KBO::compare(TermList tl1, TermList tl2) const
{
  ...
  if(tl1.isOrdinaryVar()) {
    if(existsZeroWeightUnaryFunction()) {
      ...
      return tl2.containsSubterm(tl1) ? Result::LESS : Result::INCOMPARABLE;
    }
  }
  if(tl2.isOrdinaryVar()) {
    if(existsZeroWeightUnaryFunction()) {
      ...
      return tl1.containsSubterm(tl2) ? Result::GREATER : Result::INCOMPARABLE;
    }
  }
