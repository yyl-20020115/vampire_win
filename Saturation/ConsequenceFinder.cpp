...
void ConsequenceFinder::onNewPropositionalClause(Clause* cl)
{
  ...
  TimeCounter tc(Lib::TimeCounterUnit::TC_CONSEQUENCE_FINDING);
  ...
  while(_redundantsToHandle.isNonEmpty()) {
    ...
    if(rlist) {
      ...
      while(rlist->isNonEmpty()) {
        ...
        (void)(rcl->store()!=Clause::Store::UNPROCESSED && rcl->store()!=Clause::Store::NONE);
        if(rcl->store()!=Clause::Store::PASSIVE) {
