...
void SplittingBranchSelector::considerPolarityAdvice(SATLiteral lit)
{
  ...
  case Options::SplittingLiteralPolarityAdvice::_FALSE:
      _solver->suggestPolarity(lit.var(),lit.oppositePolarity());
    break;
    case Options::SplittingLiteralPolarityAdvice::_TRUE:
      _solver->suggestPolarity(lit.var(),lit.polarity());
    break;
    case Options::SplittingLiteralPolarityAdvice::_NONE:
      // do nothing
    break;
  }
}
...
static Color colorFromPossiblyDeepFOConversion(SATClause* scl,Unit*& u)
{
  ...
  if (scl->inference()->getType() != SATInference::InfType::FO_CONVERSION) {
    ASS_EQ(scl->inference()->getType(),SATInference::InfType::PROP_INF);
  ...
  ASS_EQ(inf->getType(),SATInference::InfType::FO_CONVERSION);
    ...
    Formula* interpolant;
    {
      ...
      while(it.hasNext()) {
        ...
        if (cl->size() == 1) {
          ...
          clFla = JunctionFormula::generalJunction(Connective::OR, disjuncts);
        }
      ...
      if (conj_cnt == 1) {
        ...
        interpolant = JunctionFormula::generalJunction(Connective::AND, conjuncts);
      }
    }
  ...
  if (_ccModel) {
    ...
    if (lit && lit->isEquality() && lit->ground()) {
      if (_trueInCCModel.find(var)) {
        ASS(_solver->getAssignment(var) != SATSolver::VarAssignment::_FALSE || var > lastCheckedVar);
...
int SplittingBranchSelector::assertedGroundPositiveEqualityCompomentMaxAge()
{
  ...
  for(unsigned i=1; i<=maxSatVar; i++) {
    ...
    SATLiteral sl(i, asgn==SATSolver::VarAssignment::_TRUE);
...
SATSolver::Status SplittingBranchSelector::processDPConflicts()
{
  ...
  while (true) { // breaks inside
    {
      TimeCounter tc(Lib::TimeCounterUnit::TC_CONGRUENCE_CLOSURE);
      ...
      if(dpStatus!=DecisionProcedure::Status::UNSATISFIABLE) {
        break;
      }
    ...
    {
    	TimeCounter tca(Lib::TimeCounterUnit::TC_SAT_SOLVER);
  ...
  // ASS(_solver->getStatus()==SATSolver::SATISFIABLE);
  if (_ccModel) {
    TimeCounter tc(Lib::TimeCounterUnit::TC_CCMODEL);
    ...
    ALWAYS(_dpModel->getStatus(false) == DecisionProcedure::Status::SATISFIABLE);
    ...
    SplitLevelStack& addedComps, SplitLevelStack& removedComps)
{
  ...
  switch(asgn) {
  case SATSolver::VarAssignment::_TRUE:
    ...
    break;
  case SATSolver::VarAssignment::_FALSE:
...
void SplittingBranchSelector::recomputeModel(SplitLevelStack& addedComps, SplitLevelStack& removedComps, bool randomize)
{
  ...
  {
    TimeCounter tc1(Lib::TimeCounterUnit::TC_SAT_SOLVER);
    if (randomize) {
  ...
  if(stat == SATSolver::Status::UNKNOWN){
    ...
    throw MainLoop::MainLoopFinishedException(Statistics::TerminationReason::REFUTATION_NOT_FOUND);
  }
...
void Splitter::onAllProcessed()
{
  ...
  {
    TimeCounter tc(Lib::TimeCounterUnit::TC_SPLITTING_MODEL_UPDATE); // includes component removals and additions, also processing fast clauses and zero implied splits
...
bool Splitter::handleNonSplittable(Clause* cl)
{
  ...
  ASS_REP2(compCl->store()==Clause::Store::NONE || compCl->store()== Clause::Store::ACTIVE ||
      compCl->store()== Clause::Store::PASSIVE || compCl->store()== Clause::Store::UNPROCESSED, *compCl, compCl->store());

  if(nameRec.active && compCl->store()==Clause::Store::NONE) {
  ...
  if(sset->member(compName)) {
    ...
    Formula* f = JunctionFormula::generalJunction(Connective::OR,resLst);
...
bool Splitter::tryGetExistingComponentName(unsigned size, Literal* const * lits, SplitLevel& comp, Clause*& compCl)
{
  ...
  { 
    TimeCounter tc(Lib::TimeCounterUnit::TC_SPLITTING_COMPONENT_INDEX_USAGE);
...
Clause* Splitter::buildAndInsertComponentClause(SplitLevel name, unsigned size, Literal* const * lits, Clause* orig)
{
  ...
  if (!_defs.find(posName, def_u)) {
    ...
    Formula* def_f = new BinaryFormula(Connective::IFF,
  ...
  {
    TimeCounter tc(Lib::TimeCounterUnit::TC_SPLITTING_COMPONENT_INDEX_MAINTENANCE);
...
void Splitter::removeComponents(const SplitLevelStack& toRemove)
{
  ...
  while(blit.hasNext()) {
    ...
    while (chit.hasNext()) {
      ...
      if(ccl->store()!=Clause::Store::NONE) {
        _sa->removeActiveOrPassiveClause(ccl);
        ASS_EQ(ccl->store(), Clause::Store::NONE);
      }
  ...
  while(blit2.hasNext()) {
    ...
    while(sr->reduced.isNonEmpty()) {
      ...
      if(rcl->validReductionRecord(rrec.timestamp)) {
        ...
        ASS_EQ(rcl->store(), Clause::Store::NONE);
  ...
  while (it.hasNext()) {
    ...
    if (cl->splits()) {
      ...
      if (FormulaList::isNonEmpty(disjuncts)) {
        ...
        f = JunctionFormula::generalJunction(Connective::OR, disjuncts);
      }
    }
