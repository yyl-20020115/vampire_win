...
void SaturationAlgorithm::onActiveRemoved(Clause* c)
{
  ...
  ASS(c->store()==Clause::Store::ACTIVE);
  c->setStore(Clause::Store::NONE);
...
void SaturationAlgorithm::onPassiveRemoved(Clause* c)
{
  ...
  ASS(c->store()==Clause::Store::PASSIVE);
  c->setStore(Clause::Store::NONE);
...
simpl_start:
  ...
  cl->setStore(Clause::Store::ACTIVE);
...
void SaturationAlgorithm::addUnprocessedClause(Clause* cl)
{
  ...
  cl->setStore(Clause::Store::UNPROCESSED);
  _unprocessed->add(cl);
}
...
void SaturationAlgorithm::removeActiveOrPassiveClause(Clause* cl)
{
  ...
  case Clause::Store::PASSIVE:
  {
    TimeCounter tc(Lib::TimeCounterUnit::TC_PASSIVE_CONTAINER_MAINTENANCE);
    _passive->remove(cl);
    break;
  }
  case Clause::Store::ACTIVE:
...
void SaturationAlgorithm::addToPassive(Clause* cl)
{
  ...
  ASS_EQ(cl->store(), Clause::Store::UNPROCESSED);

  cl->setStore(Clause::Store::PASSIVE);
...
bool SaturationAlgorithm::activate(Clause* cl)
{
  ...
  if (!cl->numSelected()) {
    TimeCounter tc(Lib::TimeCounterUnit::TC_LITERAL_SELECTION);

    _selector->select(cl);
  }

  ASS_EQ(cl->store(), Clause::Store::SELECTED);
  cl->setStore(Clause::Store::ACTIVE);
  env.statistics->activeClauses++;
  _active->add(cl);
  ...
  while (_postponedClauseRemovals.isNonEmpty()) {
    ...
    if (cl->store() != Clause::Store::ACTIVE &&
	cl->store() != Clause::Store::PASSIVE) {
      continue;
    }
    removeActiveOrPassiveClause(cl);
  }
...
start:
  ...
  while (! _unprocessed->isEmpty()) {
    ...
    if (forwardSimplify(c)) {
      onClauseRetained(c);
      addToPassive(c);
      ASS_EQ(c->store(), Clause::Store::PASSIVE);
    }
    else {
      ASS_EQ(c->store(), Clause::Store::UNPROCESSED);
      c->setStore(Clause::Store::NONE);
    }
...
void SaturationAlgorithm::doOneAlgorithmStep()
{
  ...
  if (_passive->isEmpty()) {
	...
	isComplete() ? Statistics::TerminationReason::SATISFIABLE : Statistics::TerminationReason::REFUTATION_NOT_FOUND;
    ...
    if (termReason == Statistics::TerminationReason::SATISFIABLE && getOptions().proof() != Options::Proof::OFF) {
  ...
  ASS_EQ(cl->store(), Clause::Store::PASSIVE);
  cl->setStore(Clause::Store::SELECTED);
